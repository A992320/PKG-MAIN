...
if (isset($_POST['ajax_action'])) {
    ...
    /* ══ بطاقة معلومات الرخصة / إعادة التحقق اليدوي ══
       يتجاوز هذا المسار الكاش ويطلب حالة حديثة من سيرفر الرخص،
       لكنه لا يمسح آخر حالة سليمة عند عطل الشبكة. */
    if ($act === 'reload_license_info') {
        $licenseKey = (string) (getLicenseKey() ?? '');
        $previous   = is_array($_SESSION['license_info'] ?? null)
            ? $_SESSION['license_info']
            : [];

        $sendLicenseInfo = static function (
            bool $success,
            array $license,
            string $status,
            string $statusLabel,
            ...
            $expiry = trim((string) ($license['expiry_date'] ?? $license['expiration'] ?? ''));
            if ($expiry === '') {
                $expiry = in_array((string) ($license['license_type'] ?? ''), ['lifetime', 'unlimited'], true)
                    ? 'مدى الحياة'
                    : 'غير محدد';
            }
            ...
            echo json_encode([
                'success'           => $success,
                'hardware_id'       => function_exists('getMachineId') ? getMachineId() : '',
                // License ID هنا هو مفتاح التفعيل المحفوظ كما طلب مدير النظام.
                'license_id'        => function_exists('getLicenseKey') ? (string) (getLicenseKey() ?? 'غير متوفر') : 'غير متوفر',
                'status'            => $status,
                'status_label'      => $statusLabel,
                'expiration'        => $expiry,
                'max_sites'         => $maxSites,
                'license_type_name' => (string) ($license['license_type_name'] ?? 'الرخصة'),
        ...
        if ($licenseKey === '') {
            $sendLicenseInfo(false, $previous, 'expired', 'Expired', 'لا يوجد مفتاح رخصة محفوظ.');
        }

        // فحص مباشر؛ لا نمسح الكاش قبل نجاح الاتصال حتى لا تتعطل اللوحة
        // إذا كان سيرفر الرخص غير متاح مؤقتاً.
        $fresh = verifyLicenseFromServer($licenseKey);
        $serverReached = !empty($fresh['success']);
        $isValid       = !empty($fresh['valid']);

        if ($serverReached && $isValid) {
            $license = is_array($fresh['license'] ?? null) ? $fresh['license'] : [];

            // توافق مع خوادم تعيد بعض الحقول خارج مصفوفة license.
            foreach (['expiry_date', 'expiration', 'max_sites', 'sites_limit', 'site_limit',
                      'license_type', 'license_type_name', 'days_left'] as $field) {
                if (!array_key_exists($field, $license) && array_key_exists($field, $fresh)) {
                    $license[$field] = $fresh[$field];
                }
            }
            ...
            $sendLicenseInfo(true, $license, 'active', 'Active', 'تم تحديث بيانات الرخصة بنجاح.');
        }

        if ($serverReached && !$isValid) {
            if (function_exists('shashetyLicenseCacheClear')) shashetyLicenseCacheClear();
            $rejected = is_array($fresh['license'] ?? null) ? $fresh['license'] : $previous;
            $sendLicenseInfo(false, $rejected, 'expired', 'Expired',
                (string) ($fresh['error'] ?? $fresh['message'] ?? 'الرخصة منتهية أو غير صالحة.'));
        }

        // عطل اتصال: أبقِ البيانات الصحيحة الأخيرة ولا تفرّغ الكاش.
        $sendLicenseInfo(false, $previous, 'offline', 'Offline',
            (string) ($fresh['error'] ?? 'تعذّر الاتصال بسيرفر الرخص. بقيت آخر حالة سليمة محفوظة.'), true);
    }
    ...
    /* ══ Multi-Storage: ربط كل حلقة تُنشأ أو تُعدَّل بموقع ملفها ══
       (storage_id + relative_path) — دون المساس بـ stream_url. وعند النشر
       يمكن إرسال الملف تلقائياً إلى قرص حسب الإعداد أو الاختيار. */
    if (is_file(dirname(__DIR__) . '/functions/storage/bootstrap.php')) {
        require_once dirname(__DIR__) . '/functions/storage/bootstrap.php';
    }
    if (!function_exists('shsStgTag')) {
        function shsStgTag($episodeId, $url) {
            if (!function_exists('storageTagEpisode') || !$episodeId) return;
            try { storageTagEpisode((int) $episodeId, (string) $url); } catch (Throwable $e) { error_log('shsStgTag: ' . $e->getMessage()); }
        }
    }
    if (!function_exists('shsStgPublish')) {
        /**
         * بعد نشر ملف مرفوع: إرساله إلى قرص تخزين (يدوي/تلقائي) كمهمة نقل آمنة.
         * لا يفشل النشر أبداً بسبب التخزين — أسوأ حالة يبقى الملف في uploads.
         */
        function shsStgPublish($episodeId, $isMovie) {
            if (!function_exists('storagePick') || !$episodeId) return [];
            try {
                // لا يقبل هذا المسار اختيار قرص من نموذج النشر. يبقى الرفع
                // مؤقتاً في uploads ثم يختار النظام وحده وجهة مناسبة آمنة.
                $target = 'auto';
                $st = db()->prepare("SELECT storage_id, relative_path FROM episodes WHERE id=?"); $st->execute([(int) $episodeId]);
                $ep = $st->fetch(PDO::FETCH_ASSOC);
                if (!$ep || !$ep['storage_id'] || (int) $ep['storage_id'] !== storageLegacyId()) return [];
                $src = storageGet((int) $ep['storage_id'], true);
                $abs = $src ? storageAbs($src, (string) $ep['relative_path']) : null;
                $bytes = ($abs && is_file($abs)) ? (int) filesize($abs) : 0;
                storageRefreshAll();
                $pick = storagePick($isMovie ? 'movies' : 'series', $bytes);
                if (!$pick['ok']) return ['storage_warning' => 'نُشر الملف لكنه بقي في التخزين الداخلي: ' . $pick['reason']];
                if (!empty($pick['storage']['is_legacy'])) return [];
                // قد يحجز طلب آخر المساحة بين الاختيار وإنشاء المهمة. نمرّ
                // بالوجهات المرتبة كلها؛ إن لم تعد الأولى صالحة ينتقل النظام
                // تلقائياً إلى التالية، ولا يعود القرار إلى المستخدم.
                $lastError = '';
                foreach ($pick['candidates'] as $candidate) {
                    if (!empty($candidate['is_legacy'])) continue;
                    $job = storageJobCreate(['source_storage_id' => (int) $ep['storage_id'], 'dest_storage_id' => (int) $candidate['id'],
                                             'scope' => 'episode', 'scope_ref' => (string) (int) $episodeId, 'type' => 'import',
                                             'scope_label' => 'نشر تلقائي', 'created_by' => (string) ($_SESSION['admin_username'] ?? '')]);
                    if (!empty($job['ok'])) {
                        storageWorkerKick();
                        return ['storage_job' => (int) $job['job_id'], 'storage_name' => (string) $candidate['name']];
                    }
                    $lastError = (string) ($job['error'] ?? 'تعذّر إنشاء مهمة النقل');
                }
                return ['storage_warning' => 'نُشر الملف لكنه بقي في التخزين الداخلي: ' . ($lastError ?: $pick['reason'])];
            } catch (Throwable $e) {
                error_log('shsStgPublish: ' . $e->getMessage());
                return ['storage_warning' => 'نُشر الملف وبقي في التخزين الداخلي'];
            }
        }
    }

    /* ══ رفع مباشر إلى التخزين الخارجي، على دفعات صغيرة ═══════════════════
       PHP يضع أي request في مجلد مؤقت قبل أن يمررنا إليه. لذلك رفع فيلم
       واحد كبير كان يملأ قرص النظام قبل أن يصل منطق التوزيع. هنا لا يزيد
       كل request عن 4MB، ثم تكتب الدفعة مباشرة إلى التخزين المختار. */
    if (!function_exists('shsStgUploadToken')) {
        function shsStgUploadToken(): string { return bin2hex(random_bytes(24)); }
        function shsStgUploadRow(string $token): ?array {
            if (!preg_match('/^[a-f0-9]{48}$/', $token)) return null;
            try {
                $q = db()->prepare("SELECT * FROM storage_upload_sessions WHERE token=? LIMIT 1");
                $q->execute([$token]);
                return $q->fetch(PDO::FETCH_ASSOC) ?: null;
            } catch (Throwable $e) { return null; }
        }
        function shsStgUploadReady(array $r): array {
            $s = storageGet((int) $r['storage_id'], true);
            return [
                'token' => (string) $r['token'], 'filename' => basename((string) $r['rel_path']),
                'original' => (string) $r['original_name'], 'size' => (int) $r['total_bytes'],
                'storage_id' => (int) $r['storage_id'], 'storage_name' => (string) ($s['name'] ?? ''),
                'url' => $s ? storagePublicUrl($s, (string) $r['rel_path']) : '',
            ];
        }
        function shsStgUploadFinish(array $r): array {
            $s = storageGet((int) $r['storage_id'], true);
            if (!$s || !storageIsWritable($s)) return ['ok' => false, 'error' => 'قرص الوجهة لم يعد قابلاً للكتابة'];
            $part = storageAbs($s, (string) $r['part_rel']);
            $final = storageAbs($s, (string) $r['rel_path']);
            if (!$part || !$final) return ['ok' => false, 'error' => 'مسار الرفع غير آمن'];
            if (is_file($final) && (int) @filesize($final) === (int) $r['total_bytes']) {
                db()->prepare("UPDATE storage_upload_sessions SET received_bytes=?, status='ready', updated_at=NOW() WHERE token=?")
                    ->execute([(int) $r['total_bytes'], (string) $r['token']]);
                return ['ok' => true];
            }
            if (!is_file($part) || (int) @filesize($part) !== (int) $r['total_bytes']) return ['ok' => false, 'error' => 'آخر دفعة لم تكتمل'];
            if (!@rename($part, $final)) return ['ok' => false, 'error' => 'تعذّر تثبيت الملف على قرص التخزين'];
            db()->prepare("UPDATE storage_upload_sessions SET received_bytes=?, status='ready', updated_at=NOW() WHERE token=?")
                ->execute([(int) $r['total_bytes'], (string) $r['token']]);
            return ['ok' => true];
        }
        ...
        foreach($items as $it){
            ...
            if(isset($catCache[$key])){
                ...
                if(!$catId){
                    ...
                    /* نوسم القسم برقم القائمة التي أنشأته. استيراد قائمة
                       واحدة قد يُنشئ عشرات الأقسام (group-title لكل مجموعة)،
                       وبلا هذا الوسم كانت كلها تنزل على الصفحة الرئيسية
                       فوراً كبطاقات لم يخترها أحد.

                       ⚠ الوسم للأقسام الجديدة فقط: القسم الموجود سلفاً
                       (طابق الاسم في الاستعلام أعلاه) قد يكون قسماً أنشأه
                       المدير بيده ويعرضه في الرئيسية — فوسمه كمستورد كان
                       سيُخفيه عنه.

                       العمود يُضاف بترحيل مستقل، وقد لا يوجد بعد على قاعدة
                       لم يُنفَّذ عليها؛ ولا يجوز أن يسقط استيراد كامل لأجل
                       حقل وصفي، فنرجع للصيغة الأصلية ونكمل. (لا DDL هنا:
                       نحن داخل معاملة، وALTER يُنهيها ضمنياً في MySQL.) */
                    try {
                        $pdo->prepare("INSERT INTO categories (name, icon, slug, playlist_id) VALUES (?, 'fas fa-th-large', ?, ?)")
                            ->execute([$groupName, $slug, $playlistId]);
                    } catch(PDOException $e) {
                        $pdo->prepare("INSERT INTO categories (name, icon, slug) VALUES (?, 'fas fa-th-large', ?)")
                            ->execute([$groupName, $slug]);
                    }
    ...
    /* ترتيب الخطوات كان معكوساً: الفلترة كانت تسبق strtolower، والنمط
       يقبل a-z الصغيرة فقط — فكل حرف كبير كان يُحذف لا يُصغَّر:
       «The Movie Name» تصير «he-ovie-ame». الـslug يُولَّد بلاحقة uniqid
       فلم يتعطّل شيء، لكنه كان مشوّهاً في كل رابط عمل جديد.
       نصغّر أولاً، ثم نفلتر، ثم نضغط الشرطات المتكررة.
       (الصفوف القديمة تحفظ slug-ها كما هو — لا استعلام يعتمد عليه.) */
    function slugU($s){
    ...
    if ($act === 'upload_admin_music') {
        ...
        // فحص التوقيع الفعلي للملف (Magic Bytes) لا الاعتماد على الامتداد المُرسَل من العميل فقط —
        // نفس مبدأ getimagesize() المستخدم مع رفع الصور في هذا المشروع، لكن لملفات mp3:
        // إمّا وسم ID3 في البداية أو رأس إطار MPEG صالح (Frame Sync 11 بت).
        $__sig = @file_get_contents($f['tmp_name'], false, null, 0, 4);
        $__looksLikeMp3 = false;
        if ($__sig !== false && strlen($__sig) >= 3) {
            if (substr($__sig, 0, 3) === 'ID3') {
                $__looksLikeMp3 = true;
            } elseif (strlen($__sig) >= 2 && ord($__sig[0]) === 0xFF && (ord($__sig[1]) & 0xE0) === 0xE0) {
                $__looksLikeMp3 = true;
            }
        }
        if (!$__looksLikeMp3) jErr('الملف لا يبدو ملف mp3 صالحاً.');
        // Secure file name
...
// ====== START TAILSCALE PYTHON CONTROLLER (UBUNTU LINUX ONLY) ======
    ...
    if($act === 'storage_upload_init'){
        $original = trim((string) ($_POST['filename'] ?? ''));
        $bytes = (int) ($_POST['size'] ?? 0);
        $ext = strtolower(pathinfo($original, PATHINFO_EXTENSION));
        if($original === '' || $bytes <= 0) jErr('اسم الملف أو حجمه غير صالح.');
        if(!in_array($ext, ['mp4','mkv','avi','mov','webm','ts','flv'], true)) jErr('صيغة غير مدعومة.');
        if(!function_exists('storagePick')) jErr('نظام التخزين غير جاهز.');
        storageRefreshAll();
        $pick = storagePick('any', $bytes);
        if(empty($pick['ok'])) jErr('لا يوجد Storage خارجي متاح لهذا الملف: ' . ($pick['reason'] ?? '')); 
        $chosen = null;
        // لا توجد عودة إلى uploads الداخلي، حتى في حالة فشل أول قرص مرشح.
        foreach (($pick['candidates'] ?? []) as $candidate) {
            if(!empty($candidate['is_legacy'])) continue;
            $root = storagePathFor((int) $candidate['id'], 'uploads');
            if(storageEnsureDir($candidate, $root . '/.shs-parts')) { $chosen = [$candidate, $root]; break; }
        }
        if(!$chosen) jErr('لا يوجد Storage خارجي مركّب وقابل للكتابة الآن.');
        [$storage, $root] = $chosen;
        $token = shsStgUploadToken();
        $safeName = 'up_' . date('Ymd_His') . '_' . substr($token, 0, 10) . '.' . $ext;
        $rel = $root . '/' . $safeName;
        $partRel = $root . '/.shs-parts/' . $token . '.part';
        try {
            db()->prepare("INSERT INTO storage_upload_sessions
                (token,storage_id,rel_path,part_rel,original_name,total_bytes,received_bytes,status,created_by,updated_at)
                VALUES (?,?,?,?,?,?,0,'uploading',?,NOW())")
                ->execute([$token, (int) $storage['id'], $rel, $partRel, mb_substr(basename($original), 0, 255), $bytes, (string) ($_SESSION['admin_username'] ?? '')]);
        } catch(Throwable $e) { error_log('storage upload init: ' . $e->getMessage()); jErr('تعذّر حجز مساحة الرفع على القرص الخارجي.'); }
        jOk(['token' => $token, 'chunk_bytes' => 4 * 1024 * 1024, 'storage_id' => (int) $storage['id'], 'storage_name' => (string) $storage['name']]);
    }

    if($act === 'storage_upload_chunk'){
        $token = (string) ($_POST['token'] ?? ''); $offset = (int) ($_POST['offset'] ?? -1);
        $r = shsStgUploadRow($token);
        if(!$r) jErr('جلسة الرفع غير موجودة أو انتهت.');
        if($r['status'] === 'ready') jOk(shsStgUploadReady($r) + ['received' => (int) $r['total_bytes'], 'complete' => true]);
        if($r['status'] !== 'uploading') jErr('جلسة الرفع ليست قابلة للاستكمال.');
        if(empty($_FILES['chunk']) || ($_FILES['chunk']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) jErr('تعذّر استلام جزء الرفع.');
        $s = storageGet((int) $r['storage_id'], true);
        if(!$s || !storageIsWritable($s)) jErr('Storage الوجهة لم يعد متاحاً للكتابة.');
        $tmp = (string) $_FILES['chunk']['tmp_name']; $chunkBytes = (int) @filesize($tmp);
        if($chunkBytes < 1 || $chunkBytes > 5 * 1024 * 1024) jErr('حجم دفعة الرفع غير صالح.');
        $expected = (int) $r['received_bytes'];
        if($offset !== $expected) jOk(['received' => $expected, 'expected_offset' => $expected, 'complete' => false]);
        if($expected + $chunkBytes > (int) $r['total_bytes']) jErr('حجم الرفع يتجاوز الحجم المحجوز.');
        $part = storageAbs($s, (string) $r['part_rel']);
        if(!$part || !storageEnsureDir($s, dirname((string) $r['part_rel']))) jErr('تعذّر تجهيز مساحة الرفع على القرص الخارجي.');
        $out = @fopen($part, 'c+b'); $in = @fopen($tmp, 'rb');
        if(!$out || !$in) { if($out) @fclose($out); if($in) @fclose($in); jErr('تعذّر فتح دفعة الرفع.'); }
        @flock($out, LOCK_EX); @fseek($out, 0, SEEK_END);
        $actual = (int) @ftell($out);
        if($actual !== $expected) { @flock($out, LOCK_UN); @fclose($out); @fclose($in); jOk(['received' => $actual, 'expected_offset' => $actual, 'complete' => false]); }
        $written = stream_copy_to_stream($in, $out); @fflush($out); @flock($out, LOCK_UN); @fclose($out); @fclose($in);
        if((int) $written !== $chunkBytes) jErr('لم تُكتب دفعة الرفع كاملة على Storage.');
        $received = $expected + $chunkBytes;
        if($received === (int) $r['total_bytes']) {
            $r['received_bytes'] = $received;
            $done = shsStgUploadFinish($r);
            if(empty($done['ok'])) jErr($done['error'] ?? 'تعذّر إنهاء الرفع.');
            $r['status'] = 'ready';
            jOk(shsStgUploadReady($r) + ['received' => $received, 'complete' => true]);
        }
        db()->prepare("UPDATE storage_upload_sessions SET received_bytes=?, updated_at=NOW() WHERE token=? AND status='uploading'")
            ->execute([$received, $token]);
        jOk(['received' => $received, 'complete' => false]);
    }
    ...
    if($act==='upload_video'){
        ...
        storageRefreshAll(); $pick=storagePick('any',(int)$f['size']); $saved=null;
        foreach(($pick['candidates']??[]) as $s){
            if(!empty($s['is_legacy'])) continue;
            $root=storagePathFor((int)$s['id'],'uploads'); $n='up_'.uniqid().'.'.$ext;
            $dest=storageAbs($s,$root.'/'.$n);
            if($dest&&storageEnsureDir($s,$root)&&mvFile($f['tmp_name'],$dest)){ $saved=[$s,$root,$n]; break; }
        }
        if(!$saved) jErr('لا يوجد Storage خارجي صالح للرفع. لم يُستخدم قرص النظام.');
        [$s,$root,$n]=$saved;
        jOk(['filename'=>$n,'original'=>$f['name'],'url'=>storagePublicUrl($s,$root.'/'.$n),'size'=>$f['size'],'storage_id'=>(int)$s['id'],'storage_name'=>$s['name']]);
    }
    ...
    if($act==='merge_subtitle'){
        ...
        $vpath=VID_UPLOAD_DIR.$vf; $videoUrl=(string)($_POST['video_url']??'');
        if($videoUrl!==''&&function_exists('storageParseUrl')){
            $loc=storageParseUrl($videoUrl); $st=$loc?storageGet((int)$loc['storage_id'],true):null;
            if($st) $vpath=storageAbsExistingInside($st,(string)$loc['rel'])?:$vpath;
        }
        ...
        jOk(['filename'=>$vf,'url'=>$videoUrl!==''?$videoUrl:VID_UPLOAD_URL.$vf,'subtitle_url'=>$subUrl,'size'=>round(filesize($vpath)/1024/1024,2).' MB','method'=>'no_ffmpeg']);
    }
    ...
    if($act === 'save_to_shashety_auto') {
        ...
        try {
            if ($target_series > 0) {
                ...
                $stmt = $pdo->prepare("SELECT MAX(episode_number) FROM episodes WHERE series_id = ?"); $stmt->execute([$target_series]);
                ...
                shsStgTag((int) $pdo->lastInsertId(), html_entity_decode($url, ENT_QUOTES, 'UTF-8'));
    ...
    if($act==='save_video_manual'){
        ...
        try {
            if ($target_series > 0) {
                ...
                $__epId = (int) $pdo->lastInsertId(); $__isMovie = false;
            } else {
                ...
                $__epId = (int) $pdo->lastInsertId(); $__isMovie = true;
            }
            shsStgTag($__epId, $vurl);
            jOk(['url'=>$vurl] + shsStgPublish($__epId, $__isMovie));
    ...
    if($act === 'move_video_file') {
        ...
        if($is_db_series && $series_id > 0) {
            ...
            if($stmt->fetch()) {
                ...
                $nstmt = $pdo->prepare("SELECT MAX(episode_number) FROM episodes WHERE series_id = ?"); $nstmt->execute([$series_id]);
        ...
        try {
            $__st = $pdo->prepare("SELECT id FROM episodes WHERE stream_url = ?"); $__st->execute([$newDbUrl]);
            foreach ($__st->fetchAll(PDO::FETCH_COLUMN) as $__eid) { shsStgTag((int) $__eid, $newDbUrl); }
        } catch (Throwable $e) {}
        jOk(['new_url' => $newDbUrl, 'message' => 'عظيم! تم النقل ومزامنة وتغيير وجهات الخادم بشكل قطعي.']);
    }

    /* ══ Multi-Storage: كل إجراءات إدارة الأقراص والتخزين (stg_*) ══
       في ملف مستقل حتى لا يتضخم هذا الملف أكثر؛ لا يعود أبداً (jOk/jErr). */
    if (strpos((string)$act, 'stg_') === 0) {
        require __DIR__ . '/storage_handlers.php';
    }

    /* ══ تنظيم جماعي: نشر عدة فيديوهات في قسم واحد ══
       إدارة الفيديوهات كانت قائمة ملفات لا أكثر: تنشر الملف الواحد عبر
       نافذة «حفظ» ثم تكرّر العملية لكل ملف. مع عشرات الملفات المرفوعة
       يصبح ذلك عملاً يدوياً طويلاً بلا سبب — فالخطوات متطابقة والاختلاف
       الوحيد هو القسم. هنا نحدّد مجموعة ونمنحها قسمها دفعة واحدة.

       ثلاثة احترازات مقصودة:
         • نتخطّى — لا نكرّر — أي ملف منشور أصلاً (نقارن باسم الملف في
           stream_url) فلا تتضاعف الأعمال في الواجهة عند ضغط الزر مرتين.
         • نتحقق من وجود الملف على القرص فعلاً؛ الملف المحذوف يدوياً من
           المجلد كان سيُنشئ عملاً برابط ميّت.
         • كل شيء داخل معاملة واحدة: إمّا تُنشر المجموعة كاملةً أو لا شيء،
           فلا تبقى أعمال نصف منشورة عند أول خطأ. */
    if($act==='bulk_organize_videos'){
        global $pdo;
        $categoryId=(int)($_POST['category_id']??0);
        $items=json_decode((string)($_POST['items']??'[]'),true);
        if($categoryId<=0) jErr('اختر القسم أولاً');
        if(!is_array($items) || !$items) jErr('لم يتم اختيار فيديوهات');
        $items=array_slice($items,0,100);

        $categoryCheck=$pdo->prepare("SELECT id FROM categories WHERE id=? LIMIT 1");
        $categoryCheck->execute([$categoryId]);
        if(!$categoryCheck->fetchColumn()) jErr('القسم المختار غير موجود');

        // أسماء الملفات المنشورة سلفاً — للمقارنة السريعة بلا استعلام لكل ملف
        $known=[];
        foreach($pdo->query("SELECT stream_url FROM episodes")->fetchAll(PDO::FETCH_COLUMN) as $streamUrl){
            $path=parse_url((string)$streamUrl,PHP_URL_PATH);
            $name=basename($path ?: (string)$streamUrl);
        ...
        $created=0; $skipped=0; $missing=0; $__createdEps=[];

        try{
            $pdo->beginTransaction();
            $addSeries=$pdo->prepare("INSERT INTO series (category_id,name,slug,is_active) VALUES (?,?,?,1)");
            $addEpisode=$pdo->prepare("INSERT INTO episodes (series_id,episode_number,title,stream_url,subtitle_url,display_order) VALUES (?,1,?,?,?,1)");

            foreach($items as $item){
                $filename=basename((string)($item['filename']??''));
                $type=(string)($item['type']??'uploaded');
                if($filename==='' || !isset($paths[$type])){ $missing++; continue; }
                if(isset($known[$filename])){ $skipped++; continue; }
                [$directory,$urlBase]=$paths[$type];
                if(!is_file($directory.$filename)){ $missing++; continue; }

                // عنوان مقروء من اسم الملف: نزع بادئة التوليد واللواصق
                $title=pathinfo($filename,PATHINFO_FILENAME);
                $title=preg_replace('/^(vid_|merged_|vid_dl_|ep_)[a-z0-9]+_?/i','',$title);
                $title=trim(preg_replace('/[._-]+/',' ',(string)$title));
                if($title==='') $title='فيديو جديد';
                // نفس ترميز العناوين المعتمد في save_video_manual — الخلط
                // بينهما يجعل بعض الأسماء تظهر بكيانات HTML في الواجهة
                $title=htmlspecialchars(strip_tags($title));

                $slug=slugU($title).'-'.uniqid();
                $addSeries->execute([$categoryId,$title,$slug]);
                $seriesId=(int)$pdo->lastInsertId();
                $addEpisode->execute([$seriesId,$title,$urlBase.$filename,'']);
                $__newEp = (int) $pdo->lastInsertId();
                shsStgTag($__newEp, $urlBase.$filename);
                $__createdEps[] = $__newEp;
                $known[$filename]=true; $created++;
            }

            $pdo->commit();
            // بعد الالتزام فقط: إرسال كل ملف منشور إلى قرص تخزين إن طُلب ذلك
            $__stg = ['queued' => 0, 'warning' => ''];
            foreach ($__createdEps as $__e) {
                $__r = shsStgPublish($__e, true);
                if (!empty($__r['storage_job'])) $__stg['queued']++;
                elseif (!empty($__r['storage_warning'])) $__stg['warning'] = $__r['storage_warning'];
            }
            jOk(['created'=>$created,'skipped'=>$skipped,'missing'=>$missing,'storage_queued'=>$__stg['queued'],'storage_warning'=>$__stg['warning']]);
        }catch(Throwable $e){
            if($pdo->inTransaction()) $pdo->rollBack();
            jErrLog('تعذر تنظيم الفيديوهات المحددة', $e);
        }
    }
    ...
    if($act==='list_videos'){
        ...
        /* اربط الملف الفيزيائي بالعمل والقسم المنشورَين في قاعدة البيانات.
           بدون هذا الربط تبقى الصفحة قائمة أسماء ملفات: لا تعرف أيّ ملف
           منشور وأيّه ما زال معلّقاً، ولا يمكن تصفيتها بحسب القسم — وهو
           أول ما يُحتاج إليه حين تتجاوز الملفات العشرات. */
        $publishedByFile=[];
        try{
            $published=$pdo->query("SELECT e.stream_url, e.series_id, s.name AS series_name, s.poster_url,
                                           s.category_id, c.name AS category_name, c.icon AS category_icon
                                    FROM episodes e
                                    INNER JOIN series s ON s.id = e.series_id
                                    LEFT JOIN categories c ON c.id = s.category_id")->fetchAll(PDO::FETCH_ASSOC);
        }catch(PDOException $e){ $published=[]; }

        foreach($published as $record){
            $path=parse_url((string)($record['stream_url']??''),PHP_URL_PATH);
            $filename=basename($path ?: (string)($record['stream_url']??''));
            if($filename!=='' && !isset($publishedByFile[$filename])) $publishedByFile[$filename]=$record;
        }
        ...
        foreach([
            ...
            foreach(glob($d.'*.{mp4,mkv,avi,mov,webm}',GLOB_BRACE)as $f){
                ...
                $vids[]=[
                    ...
                    'category_id'=>(int)($record['category_id']??0),
    ...
    if($act==='delete_series'){
        global $pdo;
        $id=intval($_POST['id']??0);
        $__st=$pdo->prepare("SELECT stream_url, subtitle_url FROM episodes WHERE series_id=?"); $__st->execute([$id]); $eps=$__st->fetchAll(PDO::FETCH_ASSOC);
        foreach($eps as $ep){
            shashetyDeleteLocalFile($ep['stream_url']??'');
            shashetyDeleteLocalFile($ep['subtitle_url']??'');
        }
        // حذف بوستر المسلسل/الفيلم إن كان ملفاً محلياً
        $__st=$pdo->prepare("SELECT poster_url FROM series WHERE id=?"); $__st->execute([$id]); $sr=$__st->fetch(PDO::FETCH_ASSOC);
    ...
    if($act==='get_episodes'){
        ...
        $__st=$pdo->prepare("SELECT * FROM episodes WHERE series_id=? ORDER BY episode_number,display_order,id"); $__st->execute([$sid]); $rows=$__st->fetchAll(PDO::FETCH_ASSOC);
        jOk(['data'=>$rows]);
    }
    if($act==='add_episode'){
        global $pdo;
        ...
        $__epId = (int) $pdo->lastInsertId();
        shsStgTag($__epId, html_entity_decode($url, ENT_QUOTES, 'UTF-8'));
        jOk(['id'=>$__epId]);
    }
    if($act==='edit_episode'){
        global $pdo;
        ...
        shsStgTag($id, html_entity_decode($url, ENT_QUOTES, 'UTF-8'));   // الرابط قد يتغيّر يدوياً ⇒ نحدّث موقع الملف
        jOk();
    }
    ...
    // نفس مبدأ update_episodes_order أعلاه لكن لأقسام الواجهة الأمامية (شجرة الأقسام
    // في admin.php) — يُستخدم في السحب والإفلات (Drag & Drop) داخل نفس المجموعة
    // (نفس القسم الأب) فقط؛ لا يُغيَّر parent_id من هنا، ذلك يبقى عبر نموذج التعديل.
    if($act === 'update_categories_order'){
        global $pdo;
        $orders = json_decode($_POST['orders'] ?? '[]', true);
        if (is_array($orders)) {
    ...
    /* ══ مجموعات الصفحة الرئيسية (Home Groups) — CRUD متوافق مع مخطط roo ══
       يستخدم جدولَي roo: home_groups (accent_color/slug) + home_group_categories
       (جدول ربط) عبر دوال functions/home_groups.php. خلف بوابة auth + CSRF.
       الحقول من الواجهة (color/category_ids/show_on_home) تُرسَم على مخطط roo. */
    if($act === 'home_group_save'){
        global $pdo;
        if(!function_exists('homeGroupsEnsureSchema') || !homeGroupsEnsureSchema()) jErr('تعذّر تهيئة جداول المجموعات.');
        $id     = (int)($_POST['id'] ?? 0);
        $name   = trim((string)($_POST['name'] ?? ''));
        if($name === '') jErr('اسم المجموعة مطلوب.');
        $desc   = trim((string)($_POST['description'] ?? ''));
        $accent = trim((string)($_POST['color'] ?? '#38bdf8'));
        if(!preg_match('/^#[0-9a-fA-F]{6}$/', $accent)) $accent = '#38bdf8';
        $type   = trim((string)($_POST['content_type'] ?? 'auto'));
        if(!in_array($type, homeGroupsAllowedTypes(), true)) $type = 'auto';
        $isActive = (isset($_POST['show_on_home']) && (string)$_POST['show_on_home'] === '0') ? 0 : 1;
        ...
        if($ids){
            $exist = [];
            foreach($pdo->query("SELECT id FROM categories")->fetchAll(PDO::FETCH_COLUMN) as $cid){ $exist[(int)$cid]=true; }
            $ids = array_values(array_filter($ids, function($v) use($exist){ return isset($exist[(int)$v]); }));
        }
        try{
            if($id > 0){
                $slug = homeGroupsSlug($pdo, $name, $id);
                $pdo->prepare("UPDATE home_groups SET name=?, slug=?, description=?, content_type=?, accent_color=?, is_active=? WHERE id=?")
                    ->execute([$name,$slug,$desc,$type,$accent,$isActive,$id]);
                $gid = $id;
            } else {
                $slug = homeGroupsSlug($pdo, $name, 0);
                $ord  = (int)$pdo->query("SELECT COALESCE(MAX(display_order),0)+1 FROM home_groups")->fetchColumn();
                $pdo->prepare("INSERT INTO home_groups (name,slug,description,content_type,accent_color,display_order,is_active) VALUES (?,?,?,?,?,?,?)")
                    ->execute([$name,$slug,$desc,$type,$accent,$ord,$isActive]);
                $gid = (int)$pdo->lastInsertId();
            }
            $pdo->prepare("DELETE FROM home_group_categories WHERE group_id=?")->execute([$gid]);
            if($ids){
                $ins = $pdo->prepare("INSERT INTO home_group_categories (group_id,category_id,display_order) VALUES (?,?,?)");
                $o=0; foreach($ids as $cid){ $ins->execute([$gid,(int)$cid,$o++]); }
            }
            homeGroupsSetConfigured($pdo, true);
            jOk(['id'=>$gid]);
        }catch(Exception $e){ jErr('تعذّر حفظ المجموعة: '.$e->getMessage()); }
    }

    if($act === 'home_group_delete'){
        global $pdo;
        if(!function_exists('homeGroupsEnsureSchema') || !homeGroupsEnsureSchema()) jErr('تعذّر تهيئة جداول المجموعات.');
        $id = (int)($_POST['id'] ?? 0);
        if($id <= 0) jErr('معرّف المجموعة غير صالح.');
        try{
            $pdo->prepare("DELETE FROM home_group_categories WHERE group_id=?")->execute([$id]);
            $pdo->prepare("DELETE FROM home_groups WHERE id=?")->execute([$id]);
            // نُبقي الوضع الحالي كما هو (تلقائي/يدوي) ونكتفي بتحديث البصمة
            homeGroupsSetConfigured($pdo, homeGroupsIsConfigured($pdo));
            jOk(['deleted'=>$id]);
        }catch(Exception $e){ jErr('تعذّر حذف المجموعة: '.$e->getMessage()); }
    }

    /* حذف جماعي لبطاقات الصفحة الرئيسية — العرض فقط:
       يحذف صفوف المجموعات وروابطها بالأقسام، ولا يمسّ جدول categories ولا
       channels ولا series إطلاقاً. الأقسام ومحتواها تبقى كما هي. */
    if($act === 'home_groups_bulk_delete'){
        global $pdo;
        if(!function_exists('homeGroupsEnsureSchema') || !homeGroupsEnsureSchema()) jErr('تعذّر تهيئة جداول المجموعات.');
        $ids = $_POST['ids'] ?? '';
        $arr = array_values(array_filter(array_map('intval', explode(',', (string)$ids)), function($v){ return $v > 0; }));
        if(!count($arr)) jErr('لم يتم تحديد أي مجموعة');
        try{
            $delC = $pdo->prepare("DELETE FROM home_group_categories WHERE group_id = ?");
            $delG = $pdo->prepare("DELETE FROM home_groups WHERE id = ?");
            $deleted = 0;
            foreach($arr as $gid){
                $delC->execute([$gid]);
                $delG->execute([$gid]);
                if($delG->rowCount() > 0) $deleted++;
            }
            // نُبقي الوضع الحالي كما هو ونكتفي بتحديث البصمة
            homeGroupsSetConfigured($pdo, homeGroupsIsConfigured($pdo));
            jOk(['deleted'=>$deleted ?: count($arr)]);
        }catch(Exception $e){
            jErr('تعذّر حذف المجموعات: '.$e->getMessage());
        }
    }

    if($act === 'home_groups_reorder'){
        global $pdo;
        if(!function_exists('homeGroupsEnsureSchema') || !homeGroupsEnsureSchema()) jErr('تعذّر تهيئة جداول المجموعات.');
        $orders = json_decode($_POST['orders'] ?? '[]', true);
        if(!is_array($orders)) jErr('تنسيق الترتيب غير صالح.');
        try{
            $st = $pdo->prepare("UPDATE home_groups SET display_order=? WHERE id=?");
            foreach($orders as $item){ $st->execute([ intval($item['order'] ?? 0), intval($item['id'] ?? 0) ]); }
            homeGroupsSetConfigured($pdo, homeGroupsIsConfigured($pdo));
            jOk();
        }catch(Exception $e){ jErr('تعذّر حفظ الترتيب: '.$e->getMessage()); }
    }

    /* «استعادة التلقائي» = العودة إلى الوضع التلقائي للصفحة الرئيسية.
       لا يحذف أي مجموعة ولا أي قسم — فقط يُلغي اعتماد التنظيم اليدوي، فتعود
       الصفحة الرئيسية تبني بطاقاتها تلقائياً من الأقسام، وأي قسم جديد يُضاف
       لاحقاً يظهر فيها تلقائياً بلا أي خطوة إضافية. */
    if($act === 'home_groups_autoseed'){
        global $pdo;
        if(!function_exists('homeGroupsEnsureSchema') || !homeGroupsEnsureSchema()) jErr('تعذّر تهيئة جداول المجموعات.');
        try{
            homeGroupsSetConfigured($pdo, false);
            if(function_exists('cacheDelete')){ @cacheDelete('content_stamp'); @cacheDelete('site_settings'); }
            jOk(['configured'=>false,'message'=>'تمت استعادة التقسيم التلقائي — كل قسم جديد سيظهر تلقائياً في الصفحة الرئيسية (مجموعاتك محفوظة).']);
        }catch(Exception $e){ jErr('تعذّرت استعادة الوضع التلقائي: '.$e->getMessage()); }
    }

    if($act === 'home_groups_list'){
        global $pdo;
        if(!function_exists('homeGroupsEnsureSchema') || !homeGroupsEnsureSchema()) jErr('تعذّر تهيئة جداول المجموعات.');
        $state = homeGroupsRead($pdo, false);
        $out = [];
        ...
        jOk(['groups'=>$out, 'configured'=>!empty($state['configured'])]);
    }

    if($act==='delete_episode'){
        global $pdo;
        $id=intval($_POST['id']??0);
        $__st=$pdo->prepare("SELECT stream_url, subtitle_url FROM episodes WHERE id=?"); $__st->execute([$id]); $ep=$__st->fetch(PDO::FETCH_ASSOC);
        if($ep){
            shashetyDeleteLocalFile($ep['stream_url']??'');
            shashetyDeleteLocalFile($ep['subtitle_url']??'');
        }
    ...
    if($act==='upload_episode_video'){
        ...
        foreach(($pick['candidates']??[]) as $s){
            if(!empty($s['is_legacy'])) continue;
            $root=storagePathFor((int)$s['id'],'uploads'); $n='ep_'.$sid.'_'.uniqid().'.'.$ext;
            $dest=storageAbs($s,$root.'/'.$n);
            if($dest&&storageEnsureDir($s,$root)&&mvFile($f['tmp_name'],$dest)){ $saved=[$s,$root,$n]; break; }
        }
        if(!$saved) jErr('لا يوجد Storage خارجي صالح للرفع. لم يُستخدم قرص النظام.');
        [$s,$root,$n]=$saved; $url=storagePublicUrl($s,$root.'/'.$n);
        $epNum=1; if(preg_match('/[Ee]p?(\d+)|[_\s\-](\d+)\./i',$f['name'],$m)) $epNum=intval($m[1]??$m[2]??1);
        jOk(['filename'=>$n,'original'=>$f['name'],'url'=>$url,'size'=>$f['size'],'episode_number'=>$epNum,'storage_id'=>(int)$s['id'],'storage_name'=>$s['name']]);
    }
   if($act==='upload_channel_logo'){
    ...
    if($act==='get_login_logs'){
        ...
        /* الرسالة الحقيقية تُعاد كما هي: هذه نقطة للمدير وحده، وإخفاء
           السبب هو ما جعل «خطأ في التحميل» يظهر بلا أي دليل — جدول ناقص
           أو صلاحية مسلوبة يبدوان متطابقين للمستخدم. */
        try {
            ...
            jErr('تعذّر قراءة سجل الدخول: ' . $e->getMessage());
        }
        /* العناوين المحظورة فعلياً الآن (ضمن نافذة الحظر) — ليعرف المدير
           ما هو محظور ويستطيع فكّه، بدل حظر صامت بلا أي أثر مرئي. */
        $blocked = [];
        try {
            $__ttl = defined('IP_BLOCK_TTL_HOURS') ? (int) IP_BLOCK_TTL_HOURS : 24;
            $blocked = $pdo->query("SELECT ip_address, blocked_at FROM blocked_ips
                WHERE blocked_at > (NOW() - INTERVAL {$__ttl} HOUR)
                ORDER BY blocked_at DESC LIMIT 100")->fetchAll(PDO::FETCH_ASSOC);
        } catch(Exception $e){
            try { $blocked = $pdo->query("SELECT ip_address FROM blocked_ips LIMIT 100")->fetchAll(PDO::FETCH_ASSOC); }
            catch(Exception $e2){ $blocked = []; }
        }
        jOk(['logs' => $rows, 'blocked' => $blocked, 'block_ttl_hours' => defined('IP_BLOCK_TTL_HOURS') ? (int) IP_BLOCK_TTL_HOURS : 24]);
    }

    /* فك حظر عنوان IP فوراً قبل انتهاء النافذة */
    if($act==='unblock_ip'){
        global $pdo;
        $myRole = $_SESSION['admin_role'] ?? 'normal';
        if(!in_array($myRole, ['administrator','super'])) jErr('ليس لديك صلاحية');
        $ip = trim((string)($_POST['ip'] ?? ''));
        if($ip === '' || strlen($ip) > 45) jErr('عنوان IP غير صالح');
        try {
            $st = $pdo->prepare("DELETE FROM blocked_ips WHERE ip_address = ?");
            $st->execute([$ip]);
            jOk(['unblocked' => $ip, 'rows' => $st->rowCount()]);
        } catch(Exception $e){
            jErr('تعذّر فك الحظر: '.$e->getMessage());
        }
    }
...
// --- التحويل الذكي MP4 (النسخة المتكيفة كلياً مع Ubuntu/Linux و Windows) ---
    ...
    if($act === 'delete_m3u_playlist'){
        ...
        try {
            ...
            /* الأقسام التي أنشأها هذا الاستيراد تبقى فارغة بعد حذف قنواته،
               فتتكدّس في كل قائمة بلا أي شيء داخلها ويستحيل تمييزها لاحقاً.
               نحذف ما كان لهذه القائمة وحدها وخلا فعلاً من أي محتوى —
               فالقسم الذي أضاف إليه المدير قناة أو عملاً يبقى كما هو. */
            try {
                $pdo->prepare("DELETE FROM categories
                                WHERE playlist_id = ?
                                  AND id NOT IN (SELECT DISTINCT category_id FROM channels WHERE category_id IS NOT NULL)
                                  AND id NOT IN (SELECT DISTINCT category_id FROM series   WHERE category_id IS NOT NULL)
                                  AND id NOT IN (SELECT p FROM (SELECT DISTINCT parent_id AS p FROM categories WHERE parent_id IS NOT NULL) AS kids)")
                    ->execute([$id]);
            } catch(PDOException $e) { /* العمود غير موجود بعد — لا شيء لتنظيفه */ }
    ...
    /* ══ إخفاء/إظهار أقسام في الصفحة الرئيسية — عرض فقط ══
       يغيّر is_active للقسم لا أكثر: القسم يبقى، وقنواته وأفلامه ومسلسلاته
       تبقى كما هي في قاعدة البيانات، لكنه لا يظهر في index. عكسه متاح دائماً. */
    if($act === 'bulk_set_categories_visibility'){
        global $pdo;
        $ids = $_POST['ids'] ?? '';
        ...
        try{
            $updated = 0;
            $st = $pdo->prepare("UPDATE categories SET is_active = ? WHERE id = ?");
            foreach($arr as $cid){ $st->execute([$visible, $cid]); $updated += $st->rowCount() > 0 ? 1 : 0; }
            if(function_exists('cacheDelete')){ @cacheDelete('content_stamp'); @cacheDelete('site_settings'); }
            jOk(['updated'=>$updated ?: count($arr), 'visible'=>$visible]);
        }catch(Exception $e){
            jErr('تعذّر تغيير الظهور: '.$e->getMessage());
        }
    }
    ...
    if($act === 'bulk_delete_categories'){
        ...
        try {
            ...
            foreach($chunks as $ck){
                ...
                /* نُعيد أبناء الأقسام المحذوفة إلى المستوى الأعلى (parent_id=NULL)
                   قبل حذفها، فلا تبقى مراجع parent_id معلّقة تُفسد بناء الشجرة.
                   نستثني الأبناء المُدرَجين في نفس دفعة الحذف (سيُحذفون أصلاً). */
                $pdo->exec("UPDATE categories SET parent_id = NULL WHERE parent_id IN ($list) AND id NOT IN ($list)");
    ...
    function xtreamEnsureTables($pdo){
        try {
            ...
            // CREATE TABLE IF NOT EXISTS لا يُفترض أن يفشل أبداً — الوصول لهذا الـcatch
            // يعني خطأ حقيقي (صلاحيات DB، اتصال...) يستحق التسجيل لا الإخفاء الصامت.
            if (function_exists('logTo')) logTo('error', 'xtreamEnsureTables: فشل إنشاء جدول xtream_accounts', ['err' => $e->getMessage()]);
        }
        ...
        /* نوع القسم (live/movie/series) — يُحفظ صراحةً بدل استنتاجه من
           الأيقونة. عليه تُبنى شجرة Xtream في منتقي مجموعات الصفحة
           الرئيسية، وأي تعديل للأيقونة من اللوحة كان سيكسر الاستنتاج. */
        try { $pdo->exec("ALTER TABLE categories ADD COLUMN xtream_kind VARCHAR(10) NULL DEFAULT NULL"); } catch(PDOException $e){}
    ...
    function xtreamApi($base, $user, $pass, $action='', $extra=[]){
        ...
        /* ══ إعادة محاولة بتباعد متزايد ══
           كثير من لوحات Xtream تحدّ معدّل الطلبات وتردّ 429 (Too Many Requests)
           أثناء الاستيراد الطويل، أو تتعثّر مؤقتاً بـ5xx. كان أي ردّ كهذا
           يُسقط العملية كلها فوراً. الآن نعيد المحاولة حتى ٣ مرات بفاصل
           متزايد (٢ ثم ٤ ثوانٍ)، فتمرّ الموجة العابرة بلا تدخّل المستخدم. */
        $raw = false; $err = ''; $code = 0;
        $__attempts = 0; $__max = 3;
        while ($__attempts < $__max) {
            $__attempts++;
            $ch = curl_init();
            curl_setopt_array($ch, [
                CURLOPT_URL => $url,
            ...
            $__transient = ($raw === false) || $code === 429 || $code === 408 || $code >= 500;
            if (!$__transient) break;
            if ($__attempts < $__max) { sleep($__attempts * 2); }
        }
        if($raw === false) return ['_error'=>'تعذر الاتصال بالسيرفر: '.$err];
        if($code === 429) return ['_error'=>'المزوّد يحدّ الطلبات مؤقتاً (429 Too Many Requests) ولم تنجح إعادة المحاولة. انتظر دقيقة ثم أعد الاستيراد.'];
    ...
    if($act === 'xtream_import'){
        ...
        $catCache = [];
        $getCat = function($rawName, $icon, $kind = 'live') use ($pdo, $accId, &$catCache, $prefix){
            ...
            $kind = in_array($kind, ['live','movie','series'], true) ? $kind : 'live';
            /* العمود قد يغيب على قاعدة لم يُنفَّذ عليها الترحيل بعد (صلاحيات
               ALTER مثلاً). لا يجوز أن يسقط استيراد كامل لأجل حقل وصفي،
               فنرجع لصيغة بلا نوع ونكمل. */
            try {
                $pdo->prepare("INSERT INTO categories (name, icon, slug, xtream_account_id, xtream_kind) VALUES (?,?,?,?,?)")
                    ->execute([xtreamFit($catName, 450), $icon, $slug, $accId, $kind]);
            } catch(PDOException $e) {
            ...
            if($impLive){
                ...
                if(is_array($streams) && !isset($streams['_error'])){
                    ...
                    foreach($streams as $s){
                        ...
                        $catId = $getCat($cName, 'fas fa-tv', 'live');
            ...
            if($impVod){
                ...
                if(is_array($streams) && !isset($streams['_error'])){
                    ...
                    foreach($streams as $s){
                        ...
                        $catId = $getCat('🎬 '.$cName, 'fas fa-film', 'movie');
            ...
            if($impSeries){
                ...
                if(is_array($list) && !isset($list['_error'])){
                    ...
                    foreach($list as $sr){
                        ...
                        $catId = $getCat('📺 '.$cName, 'fas fa-clapperboard', 'series');
