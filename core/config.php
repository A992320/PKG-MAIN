...
/* ══ مدة حظر الـIP ══
   كان الحظر في blocked_ips دائماً بلا انتهاء ذاتي ولا وسيلة لإزالته إلا
   بتعديل قاعدة البيانات مباشرة — فإذا حُظر المدير نفسه بعد محاولات فاشلة
   فقد الوصول للوحة التي تحوي زر فك الحظر، بلا أي طريق رجوع. الجدول يحمل
   عمود blocked_at أصلاً ولم يكن يُستخدم. الآن يُقاس الحظر بنافذة زمنية:
   الرادع يبقى فعّالاً ضد التخمين الآلي، مع استعادة ذاتية مضمونة. */
if (!defined('IP_BLOCK_TTL_HOURS')) { define('IP_BLOCK_TTL_HOURS', 24); }
...
if (session_status() === PHP_SESSION_NONE && PHP_SAPI !== 'cli') {
    ...
    /* ══ تخزين الجلسات في مجلد المشروع ══
       على الاستضافة المشتركة يكون مجلد الجلسات الافتراضي مشتركاً بين كل
       المواقع: أي موقع آخر (أو منظّف النظام) قد يمسح ملفات جلساتنا فيخرج
       المدير فجأة. مجلد خاص بصلاحية 0700 يعزل جلساتنا ويطيل عمرها بأمان. */
    $__sessionTtl = 7200;
 ...
 */
/**
 * ذاكرة الطلب المشتركة بين cacheGet/cacheSet.
 *
 * كانت cacheGet تستخدم static خاصاً بها: غير محدود الحجم (يتضخّم أثناء
 * الاستيراد الطويل حتى نفاد الذاكرة)، وبلا فحص صلاحية (قيمة انتهت على
 * القرص تبقى صالحة في الذاكرة طوال الطلب)، ولا يراه cacheSet فيبقى
 * قديماً بعد الكتابة. هذه الدالة تعالج الثلاثة معاً.
 *
 * @return array
 */
function &cacheRequestMemory(): array
{
    static $memory = [];
    return $memory;
}
...
{
    $memory =& cacheRequestMemory();

    if (array_key_exists($key, $memory)) {
        $entry = $memory[$key];
        if (is_array($entry) && array_key_exists('val', $entry)) {
            if ($entry['exp'] === 0 || $entry['exp'] > time()) {
                return $entry['val'];
            }
            unset($memory[$key]); // انتهت صلاحيتها داخل الطلب نفسه
        }
    }
    ...
    if (function_exists('apcu_enabled') && apcu_enabled()) {
        ...
        if ($ok) {
            // APCu يملك مهلة هذه القيمة؛ لا نمدّدها بنسخة محلية بلا مهلة.
            cacheMemoryPut($memory, $key, $val, time() + 5);
            return $val;
        }
    }
    ...
    cacheMemoryPut($memory, $key, $payload['val'], (int) $payload['exp']);
    return $payload['val'];
}

/**
 * وضع قيمة في ذاكرة الطلب مع سقف لعدد المدخلات.
 * الإزاحة تحذف من الذاكرة فقط ولا تمسّ بيانات القرص إطلاقاً.
 *
 * @param array $memory مرجع ذاكرة الطلب.
 * @param string $key المفتاح.
 * @param mixed $val القيمة.
 * @param int $exp زمن الانتهاء (0 = دائم).
 * @return void
 */
function cacheMemoryPut(array &$memory, string $key, $val, int $exp): void
{
    if (count($memory) >= 128 && !array_key_exists($key, $memory)) {
        foreach ($memory as $oldKey => $unused) { unset($memory[$oldKey]); break; }
    }
    $memory[$key] = ['val' => $val, 'exp' => $exp];
}
...
function cacheSet(string $key, $value, int $ttl = 300): bool
{
    ...
    $expAt   = $ttl > 0 ? time() + $ttl : 0;
    $payload = serialize(['exp' => $expAt, 'val' => $value]);

    // تحديث ذاكرة الطلب فوراً حتى لا تُقرأ قيمة قديمة بعد الكتابة
    $memory =& cacheRequestMemory();
    cacheMemoryPut($memory, $key, $value, $expAt);
...
{
    // إسقاط نسخة الذاكرة أيضاً، وإلا بقيت القيمة القديمة تُقرأ بعد الحذف
    $memory =& cacheRequestMemory();
    unset($memory[$key]);
