...
window.rsLoadStatus = function(){
  return API('restream_status').then(d => {
    ...
    // نفس المبدأ لقائمة سقف الجودة — لا نلمسها أثناء تعديل المستخدم لها
    const qs = $$('rsQualitySelect');
    if (qs && document.activeElement !== qs && r.quality) qs.value = r.quality;
...
window.rsLoadDiagnostics = function(){
  const box = $$('rsDiagLog'); if (!box) return;
  box.textContent = 'جارٍ قراءة السجل…';
  API('restream_diagnostics').then(d => {
    if (!d.success) { box.textContent = 'تعذّر قراءة سجل الوسيط.'; return; }
    const rows = Array.isArray(d.entries) ? d.entries : [];
    if (!rows.length) { box.textContent = 'لا توجد أخطاء أو محاولات مسجلة اليوم.'; return; }
    box.innerHTML = rows.slice().reverse().map(r => {
      const when = String(r.time || '').replace('T',' ').replace(/\+.*$/,'');
      const c = r.ctx || {}, extra = [];
      if (c.key) extra.push('القناة: ' + c.key);
      if (c.source_host) extra.push('المصدر: ' + c.source_host);
      if (c.detail) extra.push(String(c.detail));
      return '<div style="padding:7px 0;border-bottom:1px solid var(--br,#2a2a2a)"><b>' + E(when) + '</b> — '
        + E(r.msg || '') + (extra.length ? '<br><span style="color:var(--t3,#aaa)">' + E(extra.join(' · ')) + '</span>' : '') + '</div>';
    }).join('');
  }).catch(() => { box.textContent = 'تعذّر الاتصال بسجل الوسيط.'; });
};
...
/* حفظ حدّ القنوات من اللوحة — بلا طرفية ولا sudo.
   ...
   القيد الحقيقي هو نطاق المدير لا رقم اعتباطي هنا — العدد مفتوح عملياً
   (سقف عقلاني بعيد جداً في الخادم يمنع خطأً كارثياً فقط، لا يظهر هنا). */
window.rsSaveLimit = function(){
  const li = $$('rsLimitInput'); if (!li) return;
  const n = parseInt(li.value, 10);
  if (!(Number.isInteger(n) && n >= 1)) { say('rsAlert', RS.limit_bad || 'أدخل رقماً صحيحاً أكبر من صفر', 'e'); return; }
...
/* حفظ سقف جودة إعادة الترميز (تلقائي/480/720/1080/4K).
   "تلقائي" يبقي القناة كما هي (نسخ حين يكون المصدر متوافقاً)؛ أي قيمة
   محدّدة تضمن ألا تتجاوز القناة تلك الدقة، وتفرض إعادة ترميز حقيقية
   حين يتجاوزها المصدر — كلفة معالج إضافية لتلك القنوات فقط، مقابل حجز
   نطاق ترددي أقل لكل مشاهد. */
window.rsSaveQuality = function(){
  const qs = $$('rsQualitySelect'); if (!qs) return;
  const q = qs.value;
  const btn = $$('rsQualitySaveBtn'); if (btn) btn.disabled = true;
  API('restream_set_quality', { quality: q }).then(d => {
    if (btn) btn.disabled = false;
    if (!d.success) { say('rsAlert', emsg(d), 'e'); return; }
    if (d.quality) qs.value = d.quality;
    say('rsAlert', RS.quality_saved || 'حُفظ سقف الجودة', 's');
    rsLoadStatus();
  });
};
