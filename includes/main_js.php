...
const titles={dashboard:'<?= $t["dashboard"] ?? "لوحة التحكم" ?>',categories:'<?= $t["categories"] ?? "الأقسام" ?>',channels:'<?= $t["channels"] ?? "القنوات" ?>','m3u-import':'استيراد M3U',xtream:'حساب Xtream',series:'<?= $t["series"] ?? "شاشتي" ?>',vupload:'<?= $t["upload"] ?? "رفع الأفلام" ?>',vmanage:'<?= $t["manage"] ?? "إدارة الفيديوهات" ?>','storage-mgmt':'<?= addslashes($t["storage_management"] ?? "إدارة التخزين") ?>','api-settings':'<?= $t["api_settings"] ?? "إعدادات API" ?>','site-settings':'<?= $t["settings"] ?? "إعدادات الموقع" ?>','change-password':'<?= $t["password"] ?? "كلمة المرور" ?>','system-tools':'<?= $t["tools"] ?? "صيانة النظام" ?>',backup:'<?= $t["backup"] ?? "النسخ الاحتياطي" ?>',users:'<?= $t["users"] ?? "إدارة المستخدمين" ?>','frontend-control':'التحكم بالواجهة الأمامية','general-settings':'الإعدادات العامة',subscriptions:'<?= addslashes($t["nav_plans"] ?? "خطط الاشتراك") ?>',coupons:'<?= addslashes($t["nav_coupons"] ?? "أكواد التفعيل") ?>',subscribers:'<?= addslashes($t["nav_subscribers"] ?? "المشتركون") ?>'};
...
/* إخفاء/إظهار شكل الأقسام في الصفحة الرئيسية — تغيير عرض فقط:
   لا يُحذف أي قسم ولا قناة ولا فيلم ولا مسلسل، فقط لا تظهر بطاقته في index. */
function catBulkVisibility(visible){
  const ids = [...document.querySelectorAll('.catSelChk:checked')].map(c=>c.value);
  if(!ids.length) return;
  api({ajax_action:'bulk_set_categories_visibility', ids:ids.join(','), visible:visible?1:0}).then(d=>{
    if(d && d.success){
      if(window.toast) toast((visible?'تم إظهار ':'تم إخفاء ')+(d.updated||ids.length)+' قسم في الصفحة الرئيسية (بلا حذف أي محتوى)','s');
      setTimeout(()=>location.reload(), 800);
    } else { if(window.toast) toast((d&&d.error)||'تعذّر تغيير الظهور','e'); }
  });
}
...
/* يرفع الفيديو على دفعات صغيرة ويكتب كل دفعة مباشرة على Storage المختار.
   لا يحمل قرص النظام سوى الدفعة الجارية (4MB)، ولا يختار المتصفح القرص. */
async function shsStorageUpload(file, progress){
    const init = await api({ajax_action:'storage_upload_init',filename:file.name,size:file.size});
    if(!init.success) throw new Error(init.error||'تعذّر تجهيز التخزين الخارجي');
    const size = Number(file.size), chunk = Number(init.chunk_bytes)||4194304, token = init.token;
    let offset = 0, last = null;
    while(offset < size){
        const end = Math.min(size, offset + chunk), fd = new FormData();
        if(window.csrfToken) fd.append('csrf_token',window.csrfToken);
        fd.append('ajax_action','storage_upload_chunk'); fd.append('token',token); fd.append('offset',String(offset));
        fd.append('chunk',file.slice(offset,end),file.name);
        const d = await new Promise((resolve,reject)=>{
            const x = new XMLHttpRequest();
            x.upload.onprogress = e=>{ if(e.lengthComputable && progress) progress(offset + e.loaded,size); };
            x.onload = ()=>{ try{resolve(JSON.parse(x.responseText));}catch(e){reject(new Error('استجابة الرفع غير صالحة'));} };
            x.onerror = ()=>reject(new Error('انقطع الاتصال أثناء الرفع'));
            x.open('POST',location.href); x.send(fd);
        });
        if(!d.success) throw new Error(d.error||'تعذّر كتابة دفعة الرفع');
        const received = Number(d.received);
        if(d.complete) return d;
        if(!Number.isFinite(received) || received <= offset || received > size) throw new Error('تعذّر متابعة الرفع بأمان');
        offset = received; last = d;
    }
    if(last&&last.complete) return last;
    throw new Error('لم يكتمل تثبيت الملف على Storage');
}

/* هذه التعريفات تأتي بعد النسخ القديمة عمداً لتربط واجهة المسلسلات بالرفع
   المتجزئ من دون كسر روابط الفيديو أو الواجهة الحالية. */
function epFileUpload(inp){
    const f=inp.files[0]; if(!f)return;
    $('epFilePBar').style.width='0%'; $('epFileProgress').style.display='block'; $('epFileChip').style.display='none';
    shsStorageUpload(f,(loaded,total)=>{$('epFilePBar').style.width=Math.round(loaded/total*100)+'%';})
      .then(d=>{ $('epFileProgress').style.display='none'; _epFileUpUrl=d.url; $('epUploadedUrl').value=d.url;
          $('epFileChip').style.display='flex'; $('epFileChipName').textContent=d.original||f.name;
          $('epNum').value=d.episode_number||1; if(!$('epTitle').value.trim()) $('epTitle').value=(d.original||f.name).replace(/\.[^.]+$/,''); })
      .catch(e=>{ $('epFileProgress').style.display='none'; al('addEpAlert',e.message||'فشل الرفع','e'); });
}

async function bulkUpload(){
    if(!_bulkFiles.length||!_srCurId){al('bulkAlert','تأكد من اختيار الملفات','e');return;}
    $('bulkStartBtn').disabled=true; $('bulkProgress').style.display='block'; let done=0,errs=0;
    for(let i=0;i<_bulkFiles.length;i++){
        const f=_bulkFiles[i], stat=$('bstat-'+i); stat.textContent='جارٍ الرفع إلى Storage…'; stat.className='ep-stat up';
        $('bulkCurFile').textContent=f.name.replace(/\.[^.]+$/,'');
        try{
            const d=await shsStorageUpload(f,(loaded,total)=>{const p=Math.round(loaded/total*100);$('bulkPBar').style.width=p+'%';$('bulkProgPct').textContent=p+'%';});
            const d2=await api({ajax_action:'add_episode',series_id:_srCurId,episode_number:(i+1),title:(d.original||f.name).replace(/\.[^.]+$/,''),stream_url:d.url,subtitle_url:'',duration:''});
            if(!d2.success) throw new Error(d2.error||'تعذّر تسجيل الحلقة');
            done++; stat.textContent='✅ محفوظ على '+(d.storage_name||'Storage'); stat.className='ep-stat ok';
        }catch(e){errs++;stat.textContent='❌ '+(e.message||'فشل الرفع');stat.className='ep-stat err';}
    }
    $('bulkPBar').style.width='100%'; $('bulkProgPct').textContent='100%'; $('bulkCurFile').textContent=''; $('bulkResult').style.display='block';
    $('bulkResult').innerHTML=`<div class="al ${errs?'al-e':'al-s'}" style="margin:0">تم حفظ ${done} ملف${errs?'، وتعذر '+errs:' '}.</div>`;
    $('bulkStartBtn').disabled=false; _bulkFiles=[]; if(_srCurId)loadEps();
}
...
/* تعريف لاحق يستبدل الرفع القديم الذي كان يضع الفيلم كاملاً في uploads الداخلي. */
function vidUpload(inp){
    const f=inp.files[0]; if(!f)return;
    $('vidProg').style.display='block'; $('cancelDlBtn').style.display='none'; $('vidProgSp').style.display='inline-block'; $('vidPBar').style.animation='none'; $('vidChip').style.display='none';
    shsStorageUpload(f,(loaded,total)=>{const p=Math.round(loaded/total*100);$('vidPBar').style.width=p+'%';$('vidPct').textContent=p+'%';$('vidPLabel').textContent='رفع '+fmtSz(loaded)+' / '+fmtSz(total);})
      .then(d=>{ $('vidProg').style.display='none'; VID.filename=d.filename; VID.url=d.url; VID.file=f;
          $('vidChip').style.display='flex'; $('vidChipName').textContent=d.original||f.name; $('vidChipSize').textContent=fmtSz(f.size); $('vNext1').disabled=false;
          const title=(d.original||f.name).replace(/\.[^.]+$/,'').replace(/[._\-]/g,' ').replace(/\b(720p|1080p|4k|bdrip|web|hdtv|bluray)\b/gi,'').trim();
          $('osQ').value=title; $('vChanName').value=title; al('v1alert','✅ تم رفع الفيديو إلى '+(d.storage_name||'Storage')+' تلقائياً','s'); })
      .catch(e=>{ $('vidProg').style.display='none'; al('v1alert',e.message||'فشل الرفع','e'); });
}
...
function vidSave(){
    ...
    if(VID.subFile){
        ...
        api({ajax_action:'merge_subtitle',video_file:VID.filename,video_url:VID.url,subtitle_file:VID.subFile}).then(d=>{
            if(!d.success){
...
let _vmAll=[],_vmFiltered=[],_vmCtx={},_vmMoveCtx={},_vmCurrentPage=1;
const _vmSelected=new Map();
const _vmPerPage=24;

/* تنبيه موحّد: هذه اللوحة تستخدم al() للتنبيهات داخل النوافذ، ولا يوجد
   نظام toast عام فيها. نستعمله إن وُجد لاحقاً، وإلا فـalert() — الغرض
   ألّا تصمت العملية الجماعية عن نتيجتها. */
function vmNotify(message,type){
    if(typeof toast==='function'){toast(message,type||'s');return;}
    alert(message);
}
...
function vmLoad(){
    $('vmGrid').style.display='none';$('vmEmpty').style.display='none';$('vmLoad').style.display='block';
    api({ajax_action:'list_videos'}).then(d=>{
        $('vmLoad').style.display='none';
        if(!d.success)return;
        _vmAll=d.videos||[];
        /* نظّف التحديد من الملفات التي لم تعد موجودة (حُذفت أو نُقلت)،
           وإلا أرسل الزر الجماعي أسماء ملفات لا وجود لها. */
        const available=new Set(_vmAll.map(video=>video.type+'|'+video.filename));
        _vmSelected.forEach((_,key)=>{if(!available.has(key))_vmSelected.delete(key);});
        vmUpdateBulkBar();vmRenderExplore();vmFilter();
    });
}
function vmSelectionKey(filename,type){return String(type)+'|'+String(filename);}
function vmToggleSelect(filename,type,checked){
    const key=vmSelectionKey(filename,type);
    if(checked)_vmSelected.set(key,{filename:String(filename),type:String(type)});else _vmSelected.delete(key);
    vmUpdateBulkBar();
}
function vmClearSelection(){
    _vmSelected.clear();
    document.querySelectorAll('.vm-select input').forEach(input=>{input.checked=false;});
    vmUpdateBulkBar();
}
function vmUpdateBulkBar(){
    const bar=$('vmBulkBar'),count=$('vmBulkCount');
    if(!bar||!count)return;
    count.textContent=_vmSelected.size;
    bar.style.display=_vmSelected.size?'flex':'none';
}
function vmSelectAllVisible(){
    /* «تحديد كل المعروض» لا كل الملفات: التحديد يتبع ما تراه بعد البحث
       والتصفية، وإلا نظّم المستخدم ملفات لا يراها أمامه. */
    const maxPage=Math.max(1,Math.ceil(_vmFiltered.length/_vmPerPage));
    if(_vmCurrentPage>maxPage)_vmCurrentPage=maxPage;
    const start=(_vmCurrentPage-1)*_vmPerPage;
    _vmFiltered.slice(start,start+_vmPerPage).forEach(video=>{
        _vmSelected.set(vmSelectionKey(video.filename,video.type),{filename:String(video.filename),type:String(video.type)});
    });
    document.querySelectorAll('.vm-select input').forEach(input=>{input.checked=true;});
    vmUpdateBulkBar();
}
function vmBulkOrganize(){
    const category=$('vmBulkCategory').value;
    const items=Array.from(_vmSelected.values());
    if(!items.length)return;
    if(!category){vmNotify('اختر القسم لتنظيم الفيديوهات المحددة','e');return;}
    const button=document.querySelector('#vmBulkBar .btn-p');
    if(button){button.disabled=true;button.innerHTML='<span class="sp"></span> جارٍ التنظيم…';}
    api({ajax_action:'bulk_organize_videos',category_id:category,items:JSON.stringify(items)}).then(function(response){
        if(!response.success){vmNotify(response.error||'تعذر تنظيم الفيديوهات','e');return;}
        let message='تم تنظيم '+Number(response.created||0)+' فيديو';
        if(response.skipped)message+=' — تم تجاوز '+response.skipped+' منشور مسبقاً';
        if(response.missing)message+=' — '+response.missing+' ملف غير موجود على القرص';
        vmNotify(message,'s');
        vmClearSelection();vmLoad();
    }).catch(function(){vmNotify('انقطع الاتصال أثناء التنظيم','e');})
      .finally(function(){if(button){button.disabled=false;button.innerHTML='<i class="fas fa-folder-tree"></i> تنظيم المحدد';}});
}
function vmSetCategory(category){
    const select=$('vmCategory');
    if(!select)return;
    select.value=String(category);vmFilter(true);
}
function vmCategoryScope(categoryId){
    /* اختيار قسم أب يجب أن يعرض فروعه أيضاً — وإلا بدا القسم الرئيسي
       فارغاً والمحتوى «مفقوداً» وهو في فرعٍ منه. */
    const id=String(categoryId||'');
    if(!id||id==='all'||id==='unassigned')return null;
    const categories=Array.isArray(window._vmCategories)?window._vmCategories:[];
    const children=new Map();
    categories.forEach(function(category){
        const parent=String(Number(category.parent_id)||0);
        if(!children.has(parent))children.set(parent,[]);
        children.get(parent).push(String(category.id));
    });
    const scope=new Set([id]),queue=[id];
    while(queue.length){
        const parent=queue.shift();
        (children.get(parent)||[]).forEach(function(child){if(!scope.has(child)){scope.add(child);queue.push(child);}});
    }
    return scope;
}
function vmRenderCategoryTree(){
    const host=$('vmCategoryTree');
    if(!host)return;
    const selected=String(($('vmCategory')||{}).value||'all');
    if(selected!=='all'){host.style.display='none';host.innerHTML='';return;}
    const categories=Array.isArray(window._vmCategories)?window._vmCategories:[];
    if(!categories.length){host.style.display='none';return;}
    const byParent=new Map(),known=new Set(categories.map(category=>String(category.id)));
    categories.forEach(function(category){
        let parent=String(Number(category.parent_id)||0);
        // قسم أبوه محذوف يصبح جذراً بدل أن يسقط من الشجرة كلياً
        if(parent!=='0'&&!known.has(parent))parent='0';
        if(!byParent.has(parent))byParent.set(parent,[]);
        byParent.get(parent).push(category);
    });
    byParent.forEach(list=>list.sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''),'ar')));
    const directCounts={};
    _vmAll.forEach(video=>{const id=String(Number(video.category_id)||0);if(id!=='0')directCounts[id]=(directCounts[id]||0)+1;});
    const renderBranch=function(parent,depth,seen){
        return (byParent.get(parent)||[]).map(function(category){
            const id=String(category.id);
            if(seen.has(id))return '';   // حارس دورة: أب يشير لابنه وبالعكس
            seen.add(id);
            const scope=vmCategoryScope(id)||new Set([id]);
            let count=0;scope.forEach(key=>{count+=Number(directCounts[key]||0);});
            const icon=esc(category.icon||'fas fa-folder');
            return '<button class="vm-tree-node" style="--vm-depth:'+Math.min(depth,6)+'" onclick="vmSetCategory(\''+escA(id)+'\')"><i class="'+icon+'"></i><span>'+esc(category.name||'قسم')+'</span><b>'+count+'</b><em><i class="fas fa-chevron-left"></i></em></button>'+renderBranch(id,depth+1,seen);
        }).join('');
    };
    const tree=renderBranch('0',0,new Set());
    host.innerHTML='<div class="vm-tree-head"><span><i class="fas fa-sitemap"></i> شجرة الأقسام</span><small>اختر أي قسم لعرضه مع فروعه</small></div><div class="vm-tree-scroll">'+tree+'</div>';
    host.style.display='block';
}
function vmRenderExplore(){
    const host=$('vmExplore');
    if(!host)return;
    const insights=$('vmInsights');
    const groups=new Map();let unassigned=0,organized=0;
    _vmAll.forEach(function(video){
        if(Number(video.category_id)>0){
            organized++;
            const key=String(video.category_id);
            const current=groups.get(key)||{id:key,name:video.category_name||'قسم',icon:video.category_icon||'fas fa-folder',count:0};
            current.count++;groups.set(key,current);
        }else unassigned++;
    });
    const active=($('vmCategory')||{}).value||'all';
    const chip=function(value,label,count,icon){return '<button class="vm-explore-chip '+(String(active)===String(value)?'active':'')+'" onclick="vmSetCategory(\''+escA(String(value))+'\')"><i class="'+esc(icon)+'"></i><span>'+esc(label)+'</span><b>'+Number(count||0)+'</b></button>';};
    let html=chip('all','كل المحتوى',_vmAll.length,'fas fa-compass');
    Array.from(groups.values()).sort((a,b)=>String(a.name).localeCompare(String(b.name),'ar')).forEach(function(group){html+=chip(group.id,group.name,group.count,group.icon);});
    if(unassigned)html+=chip('unassigned','غير مصنفة',unassigned,'fas fa-folder-open');
    host.innerHTML=html;
    if(insights){
        insights.innerHTML='<button class="vm-insight" onclick="vmSetCategory(\'all\')"><i class="fas fa-film"></i><span>إجمالي الملفات</span><b>'+_vmAll.length+'</b></button>'+
          '<button class="vm-insight vm-insight-good" onclick="vmSetCategory(\'all\')"><i class="fas fa-circle-check"></i><span>منظّمة في أقسام</span><b>'+organized+'</b></button>'+
          '<button class="vm-insight '+(unassigned?'vm-insight-warn':'')+'" onclick="vmSetCategory(\'unassigned\')"><i class="fas fa-folder-open"></i><span>تحتاج تنظيماً</span><b>'+unassigned+'</b></button>'+
          '<span class="vm-insight-note"><i class="fas fa-layer-group"></i> '+groups.size+' أقسام تحتوي فيديوهات</span>';
    }
}
function vmFilter(resetPage){
    const q=($('vmSearch').value||'').toLowerCase().trim(),t=$('vmType').value;
    const categoryEl=$('vmCategory');
    const c=categoryEl?categoryEl.value:'all';
    if(resetPage!==false)_vmCurrentPage=1;
    vmRenderExplore();vmRenderCategoryTree();
    const categoryScope=vmCategoryScope(c);
    _vmFiltered=_vmAll.filter(function(video){
        // البحث يشمل اسم العمل والقسم لا اسم الملف وحده — أسماء الملفات
        // مولّدة بـuniqid فالبحث فيها وحده لا يفيد
        const searchable=[video.filename,video.series_name,video.category_name].join(' ').toLowerCase();
        const categoryMatch=c==='all'||(c==='unassigned'?!Number(video.category_id):(categoryScope?categoryScope.has(String(video.category_id)):String(video.category_id)===String(c)));
        return (!q||searchable.includes(q))&&(t==='all'||video.type===t)&&categoryMatch;
    });
    vmRender(_vmFiltered);
}
function vmGoPage(page){
    const maxPage=Math.max(1,Math.ceil(_vmFiltered.length/_vmPerPage));
    _vmCurrentPage=Math.max(1,Math.min(Number(page)||1,maxPage));
    vmRender(_vmFiltered);
}
function vmRenderPagination(total){
    const node=$('vmPagination');
    if(!node)return;
    const maxPage=Math.max(1,Math.ceil(total/_vmPerPage));
    if(total<=_vmPerPage){node.style.display='none';node.innerHTML='';return;}
    const pages=[];
    for(let page=Math.max(1,_vmCurrentPage-2);page<=Math.min(maxPage,_vmCurrentPage+2);page++)pages.push(page);
    node.innerHTML='<button class="btn btn-g" '+(_vmCurrentPage===1?'disabled':'')+' onclick="vmGoPage('+(_vmCurrentPage-1)+')"><i class="fas fa-chevron-right"></i> السابق</button>'+
      pages.map(page=>'<button class="vm-page '+(page===_vmCurrentPage?'active':'')+'" onclick="vmGoPage('+page+')">'+page+'</button>').join('')+
      '<button class="btn btn-g" '+(_vmCurrentPage===maxPage?'disabled':'')+' onclick="vmGoPage('+(_vmCurrentPage+1)+')">التالي <i class="fas fa-chevron-left"></i></button>'+
      '<span>صفحة '+_vmCurrentPage+' من '+maxPage+'</span>';
    node.style.display='flex';
}
function vmPrettyTitle(video){
    if(video.series_name)return String(video.series_name);
    return String(video.filename||'فيديو جديد')
      .replace(/\.[^.]+$/,'').replace(/^(vid_|merged_|vid_dl_|ep_)[a-z0-9]+_?/i,'')
      .replace(/[._-]+/g,' ').replace(/\s+/g,' ').trim()||'فيديو جديد';
}

function vmRender(vids){
    const g=$('vmGrid'),e=$('vmEmpty');
    const organizedCount=vids.filter(video=>Number(video.category_id)>0).length;
    const maxPage=Math.max(1,Math.ceil(vids.length/_vmPerPage));
    if(_vmCurrentPage>maxPage)_vmCurrentPage=maxPage;
    const start=(_vmCurrentPage-1)*_vmPerPage;
    const pageVideos=vids.slice(start,start+_vmPerPage);

    $('vmCnt').textContent=vids.length+' فيديو · '+organizedCount+' منظّم'+(vids.length?' · عرض '+(start+1)+'–'+Math.min(start+_vmPerPage,vids.length):'');
    if(!vids.length){g.style.display='none';e.style.display='block';vmRenderPagination(0);return;}
    e.style.display='none';g.style.display='grid';

    // المسميات بالالوان تريح النفسية وتفرق المجلدات بسهولة للتحكم المستقر.
    const typeLabels = {uploaded:'رفع عام', merged:'فيديو مدمج', series:'حلقة منشورة'};
    const typeColors = {uploaded:'rgba(76,201,240,.9)', merged:'rgba(0,208,132,.9)', series:'rgba(245,166,35,.9)'};

    /* الصورة بدل <video preload="none">: العنصر السابق كان يفتح اتصالاً
       لكل بطاقة على الشاشة ليرسم إطاراً واحداً، فتثقل الصفحة بعشرات
       الملفات. البوستر المنشور (أو أيقونة القسم) يعطي نفس الغرض بلا كلفة. */
    g.innerHTML=pageVideos.map(v=>{
        const organized=Number(v.category_id)>0;
        const displayTitle=vmPrettyTitle(v);
        const categoryLabel=organized?esc(v.category_name||'قسم'):'غير مصنف';
        const categoryIcon=esc(v.category_icon||'fas fa-folder-open');
        const visual=v.poster_url
          ? '<img class="vm-poster" src="'+esc(v.poster_url)+'" alt="" loading="lazy" decoding="async" onerror="this.remove()">'
          : '<div class="vm-fallback"><i class="'+categoryIcon+'"></i><span>'+categoryLabel+'</span></div>';
        const organization=organized
          ? '<div class="vm-category"><i class="'+categoryIcon+'"></i><span>'+categoryLabel+'</span><small><i class="fas fa-circle-check"></i> منشور</small></div>'
          : '<div class="vm-category vm-category-unassigned"><i class="fas fa-triangle-exclamation"></i><span>غير مصنف — يحتاج اختيار قسم</span></div>';
        return `<div class="vmc ${organized?'vmc-organized':'vmc-unassigned'}" id="vmc-${esc(v.filename)}">
        <div class="vmt" onclick='testChannel("${escA(v.url)}","${escA(displayTitle)}")'>
            ${visual}
            <label class="vm-select" onclick="event.stopPropagation()" title="تحديد للتنظيم الجماعي"><input type="checkbox" ${_vmSelected.has(vmSelectionKey(v.filename,v.type))?'checked':''} onchange="vmToggleSelect('${escA(v.filename)}','${escA(v.type)}',this.checked)"><span><i class="fas fa-check"></i></span></label>
        ...
        <div class="vminfo">
            <div class="vmname" title="${esc(v.filename)}">${esc(displayTitle)}</div>
            <div class="vm-file-name" title="${esc(v.filename)}">${esc(v.filename)}</div>
            <div class="vmmeta"><span><i class="fas fa-hdd"></i> ${v.size_mb} MB</span><span>${esc(v.date)}</span></div>
            ${organization}
        </div>
        <div class="vmacts">
            <button class="vmb pl" onclick='testChannel("${escA(v.url)}","${escA(displayTitle)}")' title="<?= addslashes($t["js_c00aea510d"] ?? "إلعب مقطعاً مرئياً من داخل هذا الفيديو الان!") ?>"><i class="fas fa-play"></i></button>
            ...
            <button class="vmb sv" onclick="vmOpenSave('${escA(v.filename)}','${v.type}')" title="${organized?'تعديل ربط الفيديو':'اختيار القسم ونشر الفيديو'}"><i class="fas ${organized?'fa-pen-to-square':'fa-folder-plus'}"></i></button>
    ...
    vmRenderPagination(vids.length);
}
...
function vmDoSave(){
    ...
    api({
        ...
        if(d.success){
            CM('vmSaveM');
            // Multi-Storage: أُرسل الملف إلى قرص في الخلفية، أو بقي في uploads لسبب واضح
            if(d.storage_job) vmNotify('يُنقل الملف الآن إلى «'+d.storage_name+'» (مهمة #'+d.storage_job+') — تابعها من إدارة التخزين','i');
            else if(d.storage_warning) vmNotify(d.storage_warning,'e');
            /* البقاء في إدارة الفيديوهات بدل القفز لصفحة شاشتي: من ينشر
               ملفاً ينشر عادةً عدة ملفات متتابعة، والقفز كان يقطع العمل
               ويعيده يدوياً إلى هنا في كل مرة. */
            vmNotify(targetId == "0" ? 'تم نشر الفيديو في القسم المختار' : 'تم ربط الفيديو بالعمل المختار','s');
            vmLoad();
        }else al('vmSaveAlert', d.error||'<?= addslashes($t["js_6dcbd6e069"] ?? "تعذر وصول السرديات للمكتب القُدير") ?>', 'e');
    });
}

function vmDel(fn,type){if(!confirm('<?= addslashes($t["js_df84acd002"] ?? "خطر الازالة: سوف تُنسف الذكريات كاملة عن القرص السحب الثابثة الخاصة بهذا المسير (") ?>'+fn+'<?= addslashes($t["js_7569194571"] ?? ") ؟") ?>'))return;api({ajax_action:'delete_video',filename:fn,type}).then(d=>{if(d.success){const c=document.getElementById('vmc-'+fn);if(c){c.style.opacity='0';c.style.transition='all .3s';setTimeout(()=>c.remove(),300);}_vmAll=_vmAll.filter(v=>v.filename!==fn);_vmSelected.delete(vmSelectionKey(fn,type));vmUpdateBulkBar();vmFilter(false);}else alert('❌ '+(d.error||'استغاثة لملكية السرفر غير خاضعة.'));});}
...
const ALL_SECTION_DEFS = [
    ...
    {key:'storage-mgmt', name:'إدارة التخزين',     icon:'fas fa-hard-drive'},
...
function toggleThemePanel() {
    ...
    // حالة الزرّ في الشريط تُظهر أن اللوحة مفتوحة
    const top = document.getElementById('themeTopBtn');
    if (top) top.classList.toggle('on', open);
    if (open) {
...
function _closePanelOutside(e) {
    ...
    const top   = document.getElementById('themeTopBtn');
    if (!panel) return;
    /* زرّ الشريط العلوي جزء من فتح اللوحة لا نقرة «خارجها» — بدون
       استثنائه كانت النقرة نفسها تفتح اللوحة ثم تغلقها فوراً. */
    if (!panel.contains(e.target)
        && (!fab || !fab.contains(e.target))
        && (!top || !top.contains(e.target))) {
...
/* ══ شبكة أمان عامة: التقاط أي Promise مرفوضة لم تُعالَج (fetch/JSON فشل بلا .catch)
   لمنع توقف الواجهة بصمت أو ظهور خطأ في الـConsole فقط بدون تنبيه المستخدم. ══ */
(function(){
    var _lastNetToastAt = 0;
    window.addEventListener('unhandledrejection', function(ev){
        try {
            console.error('[Unhandled Promise Rejection]', ev && ev.reason);
            var now = Date.now();
            if (now - _lastNetToastAt > 4000 && typeof _adminToast === 'function') {
                _lastNetToastAt = now;
                _adminToast('حدث خطأ غير متوقع في الاتصال، حاول مرة أخرى', 'e');
            }
            ev.preventDefault();
        } catch (e) {}
    });
...
/* ══ شجرة الأقسام في لوحة التحكم: سحب وإفلات لإعادة الترتيب (Drag & Drop)
   داخل نفس المجموعة (نفس القسم الأب) فقط — بلا مكتبة خارجية، بنفس مبدأ
   ترتيب الحلقات أعلاه (_renderEpRows/_setupDragAndDrop): تعديل محلي فوري
   ثم إعادة رسم كاملة ثم حفظ في الخلفية. عند سحب قسم له أبناء، تُنقل "كتلته"
   كاملة (هو وكل أبنائه بالترتيب نفسه) حتى لا ينفصلوا عنه. ══ */
var _catTreeCache = [];
function _catTreeInit(){
    if(!Array.isArray(window.__catTreeData)) return;
    _catTreeCache = window.__catTreeData.slice();
    _setupCatDragAndDrop();
}
function _catParentName(pid){
    if(!pid) return null;
    var p = _catTreeCache.find(function(c){ return c.id===pid; });
    return p ? p.name : null;
}
function _catGroupKey(c){
    return (c.parent_id && _catTreeCache.some(function(x){return x.id===c.parent_id;})) ? c.parent_id : 0;
}
function _catBlockLength(startIdx){
    var baseDepth = _catTreeCache[startIdx]._depth || 0;
    var len = 1;
    for(var i=startIdx+1; i<_catTreeCache.length; i++){
        if((_catTreeCache[i]._depth||0) > baseDepth) len++; else break;
    }
    return len;
}
function _renderCatTreeRows(){
    var tbody = document.getElementById('catTblBody');
    if(!tbody) return;
    tbody.innerHTML = _catTreeCache.map(function(c){
        var parentKey = _catGroupKey(c);
        var indent = (c._depth||0) * 22;
        var parentName = _catParentName(c.parent_id);
        var parentBadge = parentName ? '<span class="bdg bc">'+esc(parentName)+'</span>' : '<span style="color:var(--t3);font-size:.75rem">—</span>';
        var treeArrow = (c._depth>0) ? '<span style="color:var(--t3);margin-left:4px;font-size:.8rem">└─</span>' : '';
        return '<tr draggable="true" data-cat-id="'+c.id+'" data-parent-key="'+parentKey+'" style="cursor:grab">'
            + '<td><input type="checkbox" class="catSelChk" value="'+c.id+'" onchange="catSelCtrl()" style="width:16px;height:16px;cursor:pointer;accent-color:var(--red)"></td>'
            + '<td><i class="fas fa-grip-lines" style="color:var(--t3);font-size:.95rem"></i></td>'
            + '<td style="color:var(--t3);font-size:.75rem">#'+c.id+'</td>'
            + '<td><div class="cn" style="padding-right:'+indent+'px">'+treeArrow+'<div class="nic"><i class="'+esc(c.icon)+'"></i></div><strong style="color:var(--t1)">'+esc(c.name)+'</strong></div></td>'
            + '<td>'+parentBadge+'</td>'
            + '<td><code style="font-size:.72rem;color:var(--t3);background:var(--s3);padding:2px 7px;border-radius:4px">'+esc(c.icon)+'</code></td>'
            + '<td><span class="bdg bc">'+c.channel_count+'</span></td>'
            + '<td><label class="fc-switch" style="display:inline-flex"><input type="checkbox" data-cat-id="'+c.id+'" class="catActiveToggle" '+(c.is_active===1?'checked':'')+' onchange="toggleCategoryActive(this)"><span class="fc-slider"></span></label></td>'
            + '<td><div class="acts"><button class="ib ed" onclick=\'editCat('+JSON.stringify({id:c.id,name:c.name,icon:c.icon,parent_id:c.parent_id,description:c.description||''}).replace(/'/g,"&#39;")+')\'><i class="fas fa-pen"></i></button><button class="ib dl" onclick="postDelete(\'delete_category\', \''+c.id+'\')"><i class="fas fa-trash"></i></button></div></td>'
            + '</tr>';
    }).join('');
    _setupCatDragAndDrop();
}
var _catDragSrcEl = null;
function _setupCatDragAndDrop(){
    var rows = document.querySelectorAll('#catTblBody tr[draggable="true"]');
    rows.forEach(function(row){
        row.addEventListener('dragstart', function(e){
            _catDragSrcEl = this;
            e.dataTransfer.effectAllowed = 'move';
            try{ e.dataTransfer.setData('text/plain', this.dataset.catId); }catch(err){}
            this.style.opacity = '0.4';
        });
        row.addEventListener('dragover', function(e){
            if(!_catDragSrcEl) return;
            if(_catDragSrcEl.dataset.parentKey !== this.dataset.parentKey) return; // مجموعة مختلفة: لا نسمح
            e.preventDefault();
            e.dataTransfer.dropEffect = 'move';
        });
        row.addEventListener('dragenter', function(){
            if(_catDragSrcEl && _catDragSrcEl.dataset.parentKey===this.dataset.parentKey && this!==_catDragSrcEl){
                this.style.borderTop = '2px solid var(--red)';
            }
        });
        ...
        row.addEventListener('drop', function(e){
            ...
            this.style.borderTop = '';
            if(!_catDragSrcEl || _catDragSrcEl===this) return;
            if(_catDragSrcEl.dataset.parentKey !== this.dataset.parentKey){
                if(typeof toast==='function') toast('لا يمكن نقل قسم بين مجموعات مختلفة من هنا — استخدم زر التعديل لتغيير القسم الأب');
                return;
            }
            var srcId = parseInt(_catDragSrcEl.dataset.catId, 10);
            var tgtId = parseInt(this.dataset.catId, 10);
            var srcIdx = _catTreeCache.findIndex(function(c){return c.id===srcId;});
            var tgtIdx = _catTreeCache.findIndex(function(c){return c.id===tgtId;});
            if(srcIdx<0||tgtIdx<0) return;
            var movedGroupKey = _catGroupKey(_catTreeCache[srcIdx]);
            var blockLen = _catBlockLength(srcIdx);
            var block = _catTreeCache.splice(srcIdx, blockLen);
            var newTgtIdx = _catTreeCache.findIndex(function(c){return c.id===tgtId;});
            if(newTgtIdx<0) newTgtIdx = _catTreeCache.length;
            _catTreeCache.splice.apply(_catTreeCache, [newTgtIdx, 0].concat(block));
            _renderCatTreeRows();
            _saveCatOrderToDB(movedGroupKey);
        });
...
function _saveCatOrderToDB(groupKey){
    // نرسل ترتيب المجموعة المتأثرة فقط (نفس الأب)، بترتيبها الحالي بعد النقل
    var group = _catTreeCache.filter(function(c){ return _catGroupKey(c) === groupKey; });
    var payload = group.map(function(c, i){ return {id:c.id, order:i+1}; });
    if(!payload.length) return;
    api({ ajax_action:'update_categories_order', orders: JSON.stringify(payload) }).then(function(res){
        if(!res || !res.success){ if(typeof toast==='function') toast('تعذر حفظ الترتيب الجديد'); }
    }).catch(function(){ if(typeof toast==='function') toast('تعذر الاتصال بالخادم لحفظ الترتيب'); });
}

/* ══════════════════════════════════════════════════════════════════════
   [SHS-HG] مجموعات الصفحة الرئيسية (Home Groups) — إدارة كاملة في اللوحة
   بطاقات + مُنشئ/محرّر + منتقي شجرة الأقسام مع بحث + ترتيب + استعادة تلقائية.
   يعتمد على: window.__hgGroups, window.__hgCats (يحقنهما pages/categories.php)،
   والدوال المشتركة api()/OM()/CM()/toast()/esc().
   ══════════════════════════════════════════════════════════════════════ */
(function(){
  var _hgSel = new Set();   // معرّفات الأقسام المختارة حالياً في المودال

  function hgTypeMeta(t){
    switch(t){
      case 'live':        return {label:'بث مباشر', icon:'fas fa-broadcast-tower'};
      case 'movie':       return {label:'أفلام', icon:'fas fa-film'};
      case 'series':      return {label:'مسلسلات', icon:'fas fa-tv'};
      case 'kids':        return {label:'أطفال', icon:'fas fa-child'};
      case 'sports':      return {label:'رياضة', icon:'fas fa-futbol'};
      case 'news':        return {label:'أخبار', icon:'fas fa-newspaper'};
      case 'documentary': return {label:'وثائقي', icon:'fas fa-film'};
      case 'anime':       return {label:'أنمي', icon:'fas fa-dragon'};
      case 'action':      return {label:'أكشن', icon:'fas fa-explosion'};
      case 'comedy':      return {label:'كوميديا', icon:'fas fa-face-laugh'};
      case 'drama':       return {label:'دراما', icon:'fas fa-masks-theater'};
      case 'music':       return {label:'موسيقى وحفلات', icon:'fas fa-music'};
      default:            return {label:'تلقائي', icon:'fas fa-shapes'};
    }
  }

  /* شارة الوضع: تلقائي (كل قسم جديد يظهر تلقائياً) أو يدوي (المحددة فقط) */
  window.hgRenderMode = function(){
    var el = document.getElementById('hgModeBadge');
    if(!el) return;
    var manual = !!window.__hgConfigured;
    el.className = 'hg-mode ' + (manual ? 'manual' : 'auto');
    el.innerHTML = manual
      ? '<i class="fas fa-hand-pointer"></i> الوضع: يدوي — تظهر المجموعات المحددة فقط في الصفحة الرئيسية'
      : '<i class="fas fa-bolt"></i> الوضع: تلقائي — كل قسم جديد يظهر تلقائياً في الصفحة الرئيسية';
  };

  window.hgRenderCards = function(){
    window.hgRenderMode();
    var host = document.getElementById('hgCards');
    if(!host) return;
    var groups = Array.isArray(window.__hgGroups) ? window.__hgGroups : [];
    if(!groups.length){
      var t0=document.getElementById('hgCardsTools'); if(t0) t0.style.display='none';
      host.innerHTML = window.__hgConfigured
        ? '<div class="hg-empty">الوضع يدوي ولا توجد مجموعات — الصفحة الرئيسية ستظهر فارغة. أنشئ «مجموعة جديدة» أو اضغط «استعادة التلقائي».</div>'
        : '<div class="hg-empty">الوضع تلقائي: الصفحة الرئيسية تبني بطاقاتها من الأقسام تلقائياً، وأي قسم جديد يظهر فوراً.<br>أنشئ «مجموعة جديدة» فقط إن أردت تنظيماً يدوياً مخصّصاً.</div>';
      return;
    }
    var tools=document.getElementById('hgCardsTools');
    if(tools) tools.style.display = groups.length ? 'flex' : 'none';
    host.innerHTML = groups.map(function(g){
      var m = hgTypeMeta(g.content_type);
      var color = /^#[0-9a-fA-F]{3,8}$/.test(g.color||'') ? g.color : '#38bdf8';
      var visible = (g.show_on_home && g.is_active);
      var count = (g.category_ids||[]).length;
      return ''
        + '<div class="hg-card" data-hg-card="'+g.id+'">'
        +   '<label class="hg-card-sel"><input type="checkbox" class="hgCardChk" value="'+g.id+'" onchange="hgCardSelChanged()"></label>'
        +   '<span class="hg-badge '+(visible?'on':'off')+'">'+(visible?'ظاهرة':'مخفية')+'</span>'
        +   '<div class="hg-card-ico" style="background:'+esc(color)+'22;color:'+esc(color)+'"><i class="'+esc(m.icon)+'"></i></div>'
        +   '<div class="hg-card-name">'+esc(g.name||'')+'</div>'
        +   '<div class="hg-card-meta">'+esc(m.label)+' · '+count+' اختيار</div>'
        +   '<div class="hg-card-sub">'+esc(g.description||'—')+'</div>'
        +   '<div class="hg-card-acts">'
        +     '<button class="hg-ib del" title="حذف" onclick="hgDelete('+g.id+')"><i class="fas fa-trash"></i></button>'
        +     '<button class="hg-ib" title="تأخير" onclick="hgMove('+g.id+',1)"><i class="fas fa-arrow-down"></i></button>'
        +     '<button class="hg-ib" title="تقديم" onclick="hgMove('+g.id+',-1)"><i class="fas fa-arrow-up"></i></button>'
        +     '<button class="hg-ib" title="تعديل" onclick="hgEdit('+g.id+')"><i class="fas fa-pen"></i></button>'
        +   '</div>'
        + '</div>';
    }).join('');
  };

  /* بناء منتقي الشجرة من window.__hgCats عبر parent_id، مع تحديد بحسب _hgSel */
  function hgTreeData(){
    return Array.isArray(window.__hgCats) ? window.__hgCats : [];
  }
  /* رسم صفّ قسم واحد في المنتقي — نفس الشكل في الشجرة العادية وفي أقسام
     Xtream، فالاختلاف في التجميع لا في العنصر. */
  function hgTreeRow(c, depth, sec, acc){
    var checked = _hgSel.has(c.id) ? 'checked' : '';
    return '<label class="hg-tree-item" data-name="'+escA((c.name||'').toLowerCase())+'"'
      + (sec ? ' data-sec="'+escA(sec)+'"' : '')
      + (acc ? ' data-acc="'+escA(acc)+'"' : '')
      + ' style="padding-right:'+(8+depth*18)+'px">'
      + '<input type="checkbox" value="'+c.id+'" '+checked+' onchange="hgToggleSel('+c.id+',this.checked)">'
      + '<i class="fas fa-folder-tree hg-tw-ico"></i>'
      + '<span class="hg-tw-name">'+esc(c.name||'')+'</span>'
      + '</label>';
  }

  function hgBuildTree(){
    var list = document.getElementById('hgTreeList');
    if(!list) return;
    var cats = hgTreeData();
    if(!cats.length){ list.innerHTML='<div class="hg-tree-empty">لا توجد أقسام.</div>'; return; }

    /* الأقسام المستوردة تُفصل عن الشجرة العادية: استيراد واحد يضيف عشرات
       الأقسام الجذرية، فكانت تتناثر بين أقسام المدير بلا ترتيب ويستحيل
       العثور على قسم بعينه. تُجمَّع الآن: Xtream بترتيب لوحته نفسها
       (حساب ← نوع ← أقسام)، وM3U بحسب القائمة التي أنشأتها. */
    var plain = cats.filter(function(c){ return !c.xt_acc && !c.pl_id; });
    var xt    = cats.filter(function(c){ return !!c.xt_acc; });
    var pl    = cats.filter(function(c){ return !c.xt_acc && !!c.pl_id; });

    var childrenOf = {};
    plain.forEach(function(c){
      var k = (c.parent_id && plain.some(function(x){return x.id===c.parent_id;})) ? c.parent_id : 0;
      (childrenOf[k] = childrenOf[k] || []).push(c);
    });
    var rows = [];
    var seen = {};
    (function walk(key, depth){
      if(depth>50 || !childrenOf[key]) return;
      childrenOf[key].forEach(function(c){
        if(seen[c.id]) return; seen[c.id]=true;
        rows.push(hgTreeRow(c, depth, '', ''));
        walk(c.id, depth+1);
      });
    })(0,0);
    // أي قسم لم تطله الشجرة (حلقة/أب مفقود) نعرضه كجذر
    plain.forEach(function(c){ if(!seen[c.id]) rows.push(hgTreeRow(c, 0, '', '')); });

    if(xt.length){
      var KINDS = [
        {key:'live',   label:'القنوات',   icon:'fas fa-tv'},
        {key:'movie',  label:'الأفلام',   icon:'fas fa-film'},
        {key:'series', label:'المسلسلات', icon:'fas fa-clapperboard'},
        {key:'other',  label:'غير مصنّف', icon:'fas fa-folder'}
      ];
      var accounts = {};
      xt.forEach(function(c){
        var acc = String(c.xt_acc);
        var kind = (c.xt_kind==='live'||c.xt_kind==='movie'||c.xt_kind==='series') ? c.xt_kind : 'other';
        if(!accounts[acc]) accounts[acc] = {name:(c.xt_name||('حساب Xtream #'+acc)), kinds:{}};
        (accounts[acc].kinds[kind] = accounts[acc].kinds[kind] || []).push(c);
      });

      Object.keys(accounts).sort(function(a,b){return Number(a)-Number(b);}).forEach(function(acc){
        var info = accounts[acc];
        var total = 0;
        KINDS.forEach(function(k){ total += (info.kinds[k.key]||[]).length; });
        rows.push('<div class="hg-tree-acc" data-accrow="'+escA(acc)+'">'
          + '<i class="fas fa-satellite-dish"></i><span>'+esc(info.name)+'</span>'
          + '<span class="hg-tree-acc-n">'+total+' قسم</span></div>');

        KINDS.forEach(function(k){
          var group = info.kinds[k.key] || [];
          if(!group.length) return;
          group.sort(function(a,b){ return String(a.name||'').localeCompare(String(b.name||''),'ar'); });
          var sec = 'xt'+acc+':'+k.key;
          var ids = group.map(function(c){ return c.id; }).join(',');
          rows.push('<div class="hg-tree-sec" data-secrow="'+escA(sec)+'" data-acc="'+escA(acc)+'">'
            + '<i class="'+k.icon+'"></i><span>'+esc(k.label)+' ('+group.length+')</span>'
            + '<button type="button" class="hg-mini" onclick="hgSelectSection(\''+escA(ids)+'\',true)"><i class="fas fa-check-double"></i> تحديد الكل</button>'
            + '<button type="button" class="hg-mini" onclick="hgSelectSection(\''+escA(ids)+'\',false)"><i class="fas fa-eraser"></i></button>'
            + '</div>');
          group.forEach(function(c){ rows.push(hgTreeRow(c, 2, sec, acc)); });
        });
      });
    }

    if(pl.length){
      /* قوائم M3U أبسط من Xtream: قنوات فقط، فلا حاجة لتقسيم بالنوع —
         قسم واحد لكل قائمة بأقسامها. */
      var lists = {};
      pl.forEach(function(c){
        var key = String(c.pl_id);
        if(!lists[key]) lists[key] = {name:(c.pl_name||('قائمة M3U #'+key)), items:[]};
        lists[key].items.push(c);
      });
      Object.keys(lists).sort(function(a,b){return Number(a)-Number(b);}).forEach(function(key){
        var info = lists[key];
        info.items.sort(function(a,b){ return String(a.name||'').localeCompare(String(b.name||''),'ar'); });
        var sec = 'pl'+key;
        var ids = info.items.map(function(c){ return c.id; }).join(',');
        rows.push('<div class="hg-tree-acc is-m3u" data-accrow="'+escA(sec)+'">'
          + '<i class="fas fa-file-lines"></i><span>'+esc(info.name)+'</span>'
          + '<span class="hg-tree-acc-n">'+info.items.length+' قسم</span></div>');
        rows.push('<div class="hg-tree-sec" data-secrow="'+escA(sec)+'" data-acc="'+escA(sec)+'">'
          + '<i class="fas fa-tv"></i><span>القنوات ('+info.items.length+')</span>'
          + '<button type="button" class="hg-mini" onclick="hgSelectSection(\''+escA(ids)+'\',true)"><i class="fas fa-check-double"></i> تحديد الكل</button>'
          + '<button type="button" class="hg-mini" onclick="hgSelectSection(\''+escA(ids)+'\',false)"><i class="fas fa-eraser"></i></button>'
          + '</div>');
        info.items.forEach(function(c){ rows.push(hgTreeRow(c, 2, sec, sec)); });
      });
    }
  ...
  /* تحديد/إلغاء قسم كامل من أقسام Xtream بضغطة — بديل ضغط عشرات المربعات. */
  window.hgSelectSection = function(idsCsv, on){
    String(idsCsv||'').split(',').forEach(function(raw){
      var id = parseInt(raw,10);
      if(!id) return;
      if(on) _hgSel.add(id); else _hgSel.delete(id);
      var cb = document.querySelector('#hgTreeList input[type=checkbox][value="'+id+'"]');
      if(cb) cb.checked = !!on;
    });
    hgUpdateSelCount();
  };

  window.hgToggleSel = function(id, on){ if(on) _hgSel.add(id); else _hgSel.delete(id); hgUpdateSelCount(); };

  function hgUpdateSelCount(){
    var el=document.getElementById('hgSelCount');
    if(el) el.textContent = _hgSel.size + ' محدد';
  }

  /* تحديد الكل / إلغاء التحديد — يعمل على العناصر الظاهرة فقط، فيحترم البحث:
     ابحث عن كلمة ثم «تحديد الكل» ⇒ تُحدَّد نتائج البحث وحدها. */
  window.hgSelectAllTree = function(on){
    var list=document.getElementById('hgTreeList');
    if(!list) return;
    list.querySelectorAll('.hg-tree-item').forEach(function(it){
  ...
  window.hgFilterTree = function(q){
    q = (q||'').trim().toLowerCase();
    var items = document.querySelectorAll('#hgTreeList .hg-tree-item');
    var liveSecs = {}, liveAccs = {};
    items.forEach(function(it){
      var nm = it.getAttribute('data-name')||'';
      var show = (!q || nm.indexOf(q)!==-1);
      it.style.display = show ? '' : 'none';
      if(show){
        var sec = it.getAttribute('data-sec'); if(sec) liveSecs[sec]=1;
        var acc = it.getAttribute('data-acc'); if(acc) liveAccs[acc]=1;
      }
    });
    /* عنوان القسم يبقى فقط إذا بقي تحته نتيجة — وإلا امتلأت القائمة
       بعناوين فارغة تُخفي نتائج البحث القليلة. */
    document.querySelectorAll('#hgTreeList .hg-tree-sec').forEach(function(h){
      var sec = h.getAttribute('data-secrow')||'';
      h.style.display = (!q || liveSecs[sec]) ? '' : 'none';
    });
    document.querySelectorAll('#hgTreeList .hg-tree-acc').forEach(function(h){
      var acc = h.getAttribute('data-accrow')||'';
      h.style.display = (!q || liveAccs[acc]) ? '' : 'none';
    });
  };

  function hgOpenModal(g){
    document.getElementById('hgId').value      = g ? g.id : 0;
    document.getElementById('hgName').value    = g ? (g.name||'') : '';
    document.getElementById('hgType').value    = g ? (g.content_type||'auto') : 'auto';
    document.getElementById('hgDesc').value    = g ? (g.description||'') : '';
    document.getElementById('hgColor').value   = (g && /^#[0-9a-fA-F]{3,8}$/.test(g.color||'')) ? g.color : '#38bdf8';
    document.getElementById('hgShowHome').checked = g ? !!g.show_on_home : true;
    document.getElementById('hgModalTitle').textContent = g ? 'تعديل المجموعة' : 'مجموعة جديدة';
    _hgSel = new Set(g ? (g.category_ids||[]) : []);
    var s = document.getElementById('hgTreeSearch'); if(s) s.value='';
    hgBuildTree();
    OM('hgModal');
  }
  window.hgOpenNew = function(){ hgOpenModal(null); };
  window.hgEdit = function(id){
    var g = (window.__hgGroups||[]).find(function(x){return x.id===id;});
    if(g) hgOpenModal(g);
  };

  function hgReload(){
    return api({ajax_action:'home_groups_list'}).then(function(d){
      if(d && d.success && Array.isArray(d.groups)){
        window.__hgGroups = d.groups;
        if(typeof d.configured !== 'undefined') window.__hgConfigured = !!d.configured;
        window.hgRenderCards();
      }
    });
  }

  window.hgSave = function(){
    var name = (document.getElementById('hgName').value||'').trim();
    if(!name){ if(window.toast) toast('اسم المجموعة مطلوب','e'); return; }
    var payload = {
      ajax_action:'home_group_save',
      id: document.getElementById('hgId').value||0,
      name: name,
      description: document.getElementById('hgDesc').value||'',
      color: document.getElementById('hgColor').value||'#38bdf8',
      content_type: document.getElementById('hgType').value||'auto',
      show_on_home: document.getElementById('hgShowHome').checked ? 1 : 0,
      category_ids: JSON.stringify(Array.from(_hgSel))
    };
    api(payload).then(function(d){
      if(d && d.success){
        CM('hgModal');
        window.__hgConfigured = true; // اعتماد مجموعة يعني التنظيم اليدوي
        if(window.toast) toast('تم حفظ المجموعة — الصفحة الرئيسية الآن بالتنظيم اليدوي','s');
        hgReload();
      } else {
        if(window.toast) toast((d&&d.error)||'تعذّر الحفظ','e');
      }
    });
  };

  window.hgDelete = function(id){
    if(!confirm('حذف هذه المجموعة؟ لن تُحذف الأقسام نفسها، فقط بطاقة الصفحة الرئيسية.')) return;
    api({ajax_action:'home_group_delete', id:id}).then(function(d){
      if(d && d.success){ if(window.toast) toast('تم الحذف','s'); hgReload(); }
      else if(window.toast) toast((d&&d.error)||'تعذّر الحذف','e');
    });
  };

  window.hgMove = function(id, dir){
    var arr = window.__hgGroups||[];
    var i = arr.findIndex(function(x){return x.id===id;});
    if(i<0) return;
    var j = i + dir;
    if(j<0 || j>=arr.length) return;
    var tmp = arr[i]; arr[i]=arr[j]; arr[j]=tmp;   // تبديل محلي فوري
    window.hgRenderCards();
    var orders = arr.map(function(g,idx){ return {id:g.id, order:idx+1}; });
    api({ajax_action:'home_groups_reorder', orders: JSON.stringify(orders)}).then(function(d){
      if(!d || !d.success){ if(window.toast) toast('تعذّر حفظ الترتيب','e'); hgReload(); }
    });
  };

  /* ══ تحديد بطاقات المجموعات: الكل أو فردي، ثم حذف جماعي ══
     الحذف هنا يزيل بطاقة الصفحة الرئيسية فقط — لا يُحذف أي قسم ولا قناة
     ولا فيلم ولا مسلسل من قاعدة البيانات. */
  window.hgCardSelChanged = function(){
    var boxes = document.querySelectorAll('.hgCardChk');
    var checked = document.querySelectorAll('.hgCardChk:checked');
    var n = checked.length;

    // تمييز البطاقات المحددة بصرياً
    boxes.forEach(function(cb){
      var card = cb.closest('.hg-card');
      if(card) card.classList.toggle('is-sel', cb.checked);
    });

    var cnt = document.getElementById('hgBulkCount');
    if(cnt) cnt.textContent = n + ' محدد';
    var btn = document.getElementById('hgBulkDelBtn');
    if(btn) btn.disabled = (n === 0);
    var all = document.getElementById('hgSelAllCards');
    if(all){
  ...
  window.hgSelectAllCards = function(on){
    document.querySelectorAll('.hgCardChk').forEach(function(cb){ cb.checked = !!on; });
    window.hgCardSelChanged();
  };

  window.hgBulkDelete = function(){
    var ids = Array.prototype.slice.call(document.querySelectorAll('.hgCardChk:checked')).map(function(cb){ return cb.value; });
    if(!ids.length) return;
    if(!confirm('سيتم حذف '+ids.length+' مجموعة من بطاقات الصفحة الرئيسية.\n\nلن يُحذف أي قسم أو قناة أو فيلم أو مسلسل — تختفي البطاقات فقط من index. متابعة؟')) return;
    api({ajax_action:'home_groups_bulk_delete', ids: ids.join(',')}).then(function(d){
      if(d && d.success){
        if(window.toast) toast('تم حذف '+(d.deleted||ids.length)+' بطاقة (بلا حذف أي محتوى)','s');
        hgReload().then(function(){ window.hgCardSelChanged(); });
      } else {
        if(window.toast) toast((d&&d.error)||'تعذّر الحذف','e');
      }
    });
  };

  window.hgAutoseed = function(){
    api({ajax_action:'home_groups_autoseed'}).then(function(d){
      if(d && d.success){
        window.__hgConfigured = false;
        if(window.toast) toast(d.message||'تمت استعادة التقسيم التلقائي','s');
        hgReload();
      } else if(window.toast) toast((d&&d.error)||'تعذّرت الاستعادة','e');
    });
  };

  document.addEventListener('DOMContentLoaded', function(){
    if(document.getElementById('hgCards')) window.hgRenderCards();
  });
})();
