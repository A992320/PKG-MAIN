...
function unzip_manual(string $zip_file, string $dest_dir): array {
    ...
    while ($pos < $len - 4) {
        ...
        if (!shsUpdateArchivePathSafe($name)) {
            $results['fail'][] = $name;
            continue;
        }
        $content = false;
...
/* ══════════════════════════════════════════════════════════════════════
   حمايات التحديث (مدمجة في كود المشروع)
   ══════════════════════════════════════════════════════════════════════
   ① مسار داخل الأرشيف آمن؟ يمنع Zip-Slip: مسار مطلق، صعود بالمجلدات،
      حرف قرص ويندوز، محارف تحكّم، أسماء محجوزة، أو نهاية بنقطة/مسافة.
   ② ملف محلي يجب ألا يدهسه التحديث أبداً: رفعك ومخزّنك وإعداداتك
      ورخصتك ونسخك الاحتياطية — كان التحديث يدهسها كلها.
   ③ هل الوجهة (أو أحد آبائها) رابط رمزي؟ الكتابة عبره تخرج خارج المجلد. */
function shsUpdateArchivePathSafe(string $name): bool
{
    if ($name === '' || $name[0] === '/' || strpos($name, "\0") !== false
        || strpos($name, ':') !== false || strpos($name, '\\') !== false) return false;
    foreach (explode('/', rtrim($name, '/')) as $part) {
        if ($part === '' || $part === '.' || $part === '..') return false;
        if (preg_match('/[\x00-\x1f]/', $part)) return false;
...
function shsUpdateKeepLocal(string $path): bool
{
    $path = strtolower(str_replace('\\', '/', $path));
    if (preg_match('#(^|/)(?:\.git|\.svn)(?:/|$)#', $path)) return true;
    if (preg_match('#^(?:uploads|storage|cache|backups|_backups|_archive)(?:/|$)#', $path)) return true;
    if (preg_match('#(^|/)\.env(?:\.|$)#', $path)) return true;
    if (preg_match('#\.(?:rar|7z|bak|backup)$#', $path)) return true;
    return in_array($path, [
        'version.txt', 'channel.txt', 'license_key.txt', 'database.sql',
        '.hwid_primary.dat', '.last_verify.dat', '.system_id.dat',
    ], true);
}

function shsUpdateLinkedTarget(string $path, string $root): bool
{
    $root = rtrim($root, '/\\');
    $guard = 0;
    while ($path !== $root && $guard++ < 64) {
...
function copy_to_site(string $source_dir): array {
    ...
    foreach ($it as $item) {
        ...
        /* لا ندهس ملفات المستخدم المحلية، ولا نكتب عبر رابط رمزي */
        if (shsUpdateKeepLocal($rel)) { $skipped++; continue; }
        if ($item->isLink() || shsUpdateLinkedTarget($dest, SITE_DIR)) { $skipped++; continue; }
        $dd=dirname($dest);
