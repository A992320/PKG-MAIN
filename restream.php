...
function rxJson(array $p, int $code = 200): void {
    /* كل جواب فاشل يظهر في سجل الوسيط؛ لا نسجل الرابط الأصلي أو رموز الوصول. */
    if ($code >= 400 && function_exists('rsLog')) {
        rsLog('طلب الوسيط رُفض أو فشل', [
            'status' => $code,
            'error'  => (string)($p['error'] ?? 'unknown'),
            'kind'   => !empty($_GET['ep']) ? 'episode' : 'channel',
            'id'     => (int)($_GET['ep'] ?? $_GET['ch'] ?? 0),
        ]);
    }
