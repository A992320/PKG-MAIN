...
:root {
  ...
  /* أسماء موحّدة إضافية (LTS) — لا تكسر أي شيء، تشير لنفس القيم أعلاه */
  --bg-primary:var(--bg); --bg-secondary:var(--bg2); --surface-hover:var(--bg3);
  --text-primary:var(--text); --text-secondary:var(--text-dim);
  --success:#46d369; --warning:#fde047; --danger:#ff4d57;
}
...
/* ══════════════════════════════════════════════════════════════════════
   [SHS-DOCK] شريط الأقسام (Dock) — القوائم المنسدلة لكل مجموعة
   ══════════════════════════════════════════════════════════════════════ */
.cat-dock-trigger{position:relative}
.cat-dock-caret{
  display:inline-flex;align-items:center;justify-content:center;
  line-height:0;margin-inline-start:2px;transition:transform .2s ease;opacity:.8;
}
.cat-dock-caret svg{width:.85em;height:.85em}
.cat-dock-trigger.cat-dock-open{
  background:rgba(229,9,20,.16)!important;
  border-color:rgba(229,9,20,.5)!important;color:#fff!important;
}
.cat-dock-trigger.cat-dock-open .cat-dock-caret{transform:rotate(180deg)}

/* القائمة المنسدلة — ملحقة بـ body بموضع ثابت (لا تُقصّ داخل الشريط) */
.cat-dock-menu{
  ...
  padding:8px;
  background:linear-gradient(180deg,rgba(24,24,28,.98),rgba(16,16,19,.98));
  border:1px solid rgba(255,255,255,.1);
  border-radius:14px;
  box-shadow:0 18px 44px rgba(0,0,0,.5);
  -webkit-backdrop-filter:blur(16px) saturate(160%);
  backdrop-filter:blur(16px) saturate(160%);
  animation:catDockIn .16s ease;
}
@keyframes catDockIn{from{opacity:0;transform:translateY(-6px)}to{opacity:1;transform:translateY(0)}}
.cat-dock-menu::-webkit-scrollbar{width:8px}
.cat-dock-menu::-webkit-scrollbar-thumb{background:rgba(255,255,255,.14);border-radius:99px}
.cat-dock-item{
...
.cat-dock-item:hover,.cat-dock-item:focus-visible{
  background:rgba(255,255,255,.08);color:#fff;outline:none;
}
.cat-dock-item .lcn{
  display:inline-flex;align-items:center;justify-content:center;
  width:26px;height:26px;flex-shrink:0;border-radius:7px;
  background:rgba(229,9,20,.12);color:var(--red);line-height:0;
}
.cat-dock-item .lcn svg{width:.95em;height:.95em;stroke:currentColor;fill:none}
.cat-dock-item-name{flex:1 1 auto;min-width:0;overflow:hidden;text-overflow:ellipsis}
.cat-dock-item-count{
  flex:0 0 auto;font-size:.7rem;font-weight:800;color:var(--text-muted);
  background:rgba(255,255,255,.06);padding:2px 8px;border-radius:99px;
}
.cat-dock-item--all{color:#fff}
.cat-dock-item--all .cat-dock-item-name{font-weight:800}

/* الجوال: عناصر أكبر لسهولة اللمس، وقائمة أوسع */
@media(max-width:600px){
  .cat-dock-menu{min-width:220px;max-width:calc(100vw - 16px);padding:6px}
  .cat-dock-item{padding:12px 12px;font-size:.9rem}
}

/* ══════════════════════════════════════════════════════════════════════
   [SHS-HG] بطاقات الصفحة الرئيسية — بترتيب roo: بوستر كطبقة خلفية خافتة،
   أيقونة في الأعلى، نص في الأسفل، سهم. بحجم مضبوط لا يطغى على الصفحة.
   ══════════════════════════════════════════════════════════════════════ */
.shs-hg-wrap{padding:8px 20px 24px}
.shs-hg-head{margin:6px 0 18px;text-align:center}
.shs-hg-h2{font-size:clamp(1.3rem,2.6vw,1.8rem);font-weight:900;color:#fff;margin:0 0 6px}
.shs-hg-p{font-size:.82rem;color:var(--text-muted);margin:0}

.shs-hg-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:14px}

.shs-hg-card{
  position:relative;display:flex;align-items:flex-end;gap:14px;
  min-width:0;min-height:190px;padding:21px;overflow:hidden;isolation:isolate;
  border:1px solid rgba(255,255,255,.09);border-radius:20px;
  background:linear-gradient(145deg,rgba(255,255,255,.05),#0c121c 65%,#090d14);
  box-shadow:0 16px 34px rgba(0,0,0,.28),inset 0 1px rgba(255,255,255,.065);
  color:#fff;text-align:right;font-family:inherit;cursor:pointer;
  transition:transform .2s ease,border-color .2s ease,box-shadow .2s ease;
}
.shs-hg-card:hover{transform:translateY(-5px);border-color:var(--hg-color,#38bdf8);box-shadow:0 23px 44px rgba(0,0,0,.38)}
.shs-hg-card:focus-visible{outline:3px solid #fff;outline-offset:4px}
.shs-hg-card:active{transform:translateY(-2px)}

/* طبقة البوستر: خلف كل شيء، خافتة، مع تدرّج يضمن قراءة النص */
.shs-hg-art{
  position:absolute;z-index:-3;inset:-3%;border-radius:inherit;pointer-events:none;
  background-position:center 24%;background-repeat:no-repeat;background-size:cover;
  opacity:.67;filter:saturate(1.06) contrast(1.03) brightness(.82);
}
.shs-hg-card.has-artwork{background:linear-gradient(145deg,rgba(255,255,255,.03),#080d14 76%)}
.shs-hg-card.has-artwork::after{
  content:'';position:absolute;z-index:-2;inset:0;border-radius:inherit;pointer-events:none;
  background:linear-gradient(180deg,rgba(4,8,14,.06) 0%,rgba(5,9,15,.30) 42%,rgba(5,9,15,.94) 100%);
}
.shs-hg-glow{
  position:absolute;z-index:-1;top:-70px;inset-inline-start:-48px;width:190px;height:190px;
  border-radius:50%;background:var(--hg-color,#38bdf8);opacity:.20;filter:blur(34px);pointer-events:none;
}

/* الأيقونة: صندوق في أعلى بداية البطاقة (نفس ترتيب roo) */
.shs-hg-ico{
  position:absolute;z-index:1;top:20px;inset-inline-start:20px;
  display:grid;place-items:center;width:74px;height:74px;border-radius:22px;
  border:1px solid rgba(255,255,255,.14);
  background:linear-gradient(145deg,rgba(255,255,255,.10),rgba(7,12,20,.45));
  color:var(--hg-color,#38bdf8);
  box-shadow:inset 0 1px rgba(255,255,255,.20);
  -webkit-backdrop-filter:blur(9px);backdrop-filter:blur(9px);
}
.shs-hg-ico svg{width:34px;height:34px;stroke:currentColor;fill:none}

.shs-hg-body{position:relative;z-index:1;display:flex;flex:1;min-width:0;flex-direction:column;gap:5px}
/* unicode-bidi:plaintext — الصفحة كلها dir="rtl"، وأسماء الأقسام المستوردة من
   Xtream غالباً إنجليزية بالكامل (COLLECTION, BANGLA, ARABIC...). بلا هذا
   السطر يقرّر المتصفح جهة القصّ (…) من اتجاه الصفحة (rtl) لا من اتجاه النص
   الفعلي، فيقصّ بداية الاسم الإنجليزي ويُبقي نهايته — "COLLECTION" تظهر
   "...LLECTION" بدل "COLLECTION...". plaintext يجعل كل بطاقة تُحدَّد اتجاهها
   من أول حرف فيها فعلياً، فيصحّ القصّ للاسم الإنجليزي والعربي معاً بلا فرق. */
/* أسماء أقسام Xtream قد تكون طويلة جداً. نعرضها حتى ثلاثة أسطر بدلاً من
   إخفائها في سطر واحد، وبذلك يظل اسم المجموعة واضحاً على الشاشات الكبيرة
   والصغيرة من دون تغيير حجم البطاقة أو بيانات القسم. */
.shs-hg-name{overflow:hidden;color:#fff;font-size:1.05rem;font-weight:900;line-height:1.35;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:3;text-shadow:0 2px 8px rgba(0,0,0,.55);unicode-bidi:plaintext}
.shs-hg-meta{color:rgba(219,231,245,.72);font-size:.7rem;font-weight:800;text-shadow:0 1px 6px rgba(0,0,0,.5)}

.shs-hg-arrow{
  position:relative;z-index:1;display:grid;place-items:center;
  width:30px;height:30px;flex:0 0 30px;border-radius:10px;
  border:1px solid rgba(255,255,255,.11);background:rgba(255,255,255,.055);color:rgba(255,255,255,.82);
}
.shs-hg-arrow svg{width:16px;height:16px}

/* الجوال: عمودان، بطاقات أصغر — مطابق لمقاسات roo */
@media(max-width:600px){
  .shs-hg-wrap{padding:6px 14px 18px}
  .shs-hg-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}
  .shs-hg-card{min-height:166px;padding:13px 11px;border-radius:15px;gap:7px}
  .shs-hg-ico{top:13px;inset-inline-start:12px;width:55px;height:55px;border-radius:17px}
  .shs-hg-ico svg{width:26px;height:26px}
  .shs-hg-name{font-size:.9rem;white-space:normal;line-height:1.3}
  .shs-hg-meta{font-size:.63rem}
  .shs-hg-arrow{display:none}
}
@media(prefers-reduced-motion:reduce){
  .shs-hg-card{transition:none}
  .shs-hg-card:hover{transform:none}
}

/* [SHS-HG] حالة فراغ الرئيسية حين لا توجد مجموعات معرّفة */
.shs-hg-empty{max-width:520px;margin:40px auto;text-align:center;padding:36px 24px;border:1px dashed var(--border);border-radius:18px;background:rgba(255,255,255,.02)}
.shs-hg-empty-ico{color:var(--text-muted);opacity:.7;margin-bottom:14px;display:flex;justify-content:center}
.shs-hg-empty-title{font-size:1.15rem;font-weight:800;color:#fff;margin-bottom:8px}
.shs-hg-empty-sub{font-size:.86rem;color:var(--text-muted);line-height:1.7;margin:0}

/* [SHS-SAFE] حواف الشاشة على الهواتف ذات الحوّاف المنحنية/شريط الإيماءات:
   نحترم safe-area فلا يلتصق المحتوى بالحافة ولا يختبئ خلف شريط الإيماءات. */
@supports (padding: max(0px)) {
  ...
  @media(max-width:600px){
    ...
    #homeGroupSection,#categoryViewSection,#searchViewSection,#epSection{
      padding-inline-start:max(20px, env(safe-area-inset-left))!important;
      padding-inline-end:max(20px, env(safe-area-inset-right))!important;
    }
  }
}
