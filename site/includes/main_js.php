...
(function(){
  ...
  function currentScreen(){
    ...
    /* شاشة المجموعة الداخلية (مثل «بث عام») ليست الرئيسية العامة.
       لا بد من تمييزها كي لا يعيد التحديث المتأخر المستخدم إلى بطاقات
       أنواع المحتوى بعد إغلاق المشغّل. */
    if(vis('homeGroupSection'))     return 'group';
    return 'home';
  }
  ...
  async function refreshCurrentScreen(){
    try{
      ...
      if(scr==='group' && window.App && App.currentHomeGroup){
        /* إعادة جلب البيانات لا تعني فقدان المكان الذي كان المستخدم فيه.
           نحفظ هوية المجموعة وموضعه، ثم نرسم المجموعة نفسها من البيانات
           الحديثة بلا إضافة خطوة جديدة إلى سجل الرجوع. */
        var groupId=App.currentHomeGroup.id;
        var groupScrollY=window.scrollY||0;
        if(typeof loadAndBuildNetflixHome==='function'){
          ...
          var group=(App.homeGroups||[]).find(function(g){ return String(g.id)===String(groupId); });
          if(group && typeof shsRenderGroupView==='function') shsRenderGroupView(group,{restore:true,scrollY:groupScrollY});
        }
        return;
      }
...
const App={
  ...
  contentTree:null, /* [SHS-TREE] شجرة الأقسام المبنية من parent_id — بلا أي شبكة إضافية */
  navStack:[],       /* [SHS-TREE] مسار التنقّل الحالي داخل الشجرة (Breadcrumb + رجوع) */
...
const HIDE_FAVORITES    = <?php echo $hide_favorites ? 'true' : 'false'; ?>;
async function loadAndBuildNetflixHome(){
  ...
  try{
    ...
    /* تُبنى البطاقات في المتصفح من App.cats نفسها (مبدأ roo): لا طلب إضافي،
       ولا اعتماد على توليد من الخادم — فما دام هناك محتوى تظهر البطاقات.
       الوضع اليدوي (بعد تعريف مجموعات) يَجُبّ التلقائي.

       ⚠ الترتيب مقصود: وضع الصفحة وتعريفات المجموعات تُقرأ قبل بناء شريط
       Dock، لأن الشريط يحتاجها ليعرف أيّ أقسام Xtream سمح لها المدير
       بالظهور. لو بُني الشريط أولاً لاختفت كلها حتى المضمومة لمجموعة. */
    App.homeGroupsConfigured = (catData.home_groups_configured===true || catData.home_groups_configured===1);
    App.homeGroupDefs = Array.isArray(catData.home_groups) ? catData.home_groups : [];

    renderCategoryNavBar();
    const _hgGroups = shsComputeHomeGroups();
    App.homeGroups=_hgGroups;
    if(_hgGroups.length){
      shsRenderHomeGroups(_hgGroups);
      const _hadStateG = !!(window.location.hash||'').replace(/^#/,'');
      if(_hadStateG){
        const _hw=document.getElementById('heroWelcome'), _ns=document.getElementById('netflixStyleSliders');
        if(_hw) _hw.classList.add('hidden'); if(_ns) _ns.classList.add('hidden');
        let _okG=false; try{ _okG=await shsRestoreFromHash(); }catch(e){ console.error('restore:',e); }
        if(!_okG){ try{ shsSetHash(null); }catch(e){} if(_hw) _hw.classList.remove('hidden'); if(_ns) _ns.classList.remove('hidden'); }
      } else {
        try{ await shsRestoreFromHash(); }catch(e){ console.error('restore:',e); }
      }
      const syncDelayG=window.requestIdleCallback||(fn=>setTimeout(fn,4000));
      syncDelayG(()=>syncNotifications(App.cats));
      return; // الرئيسية الآن بطاقات مجموعات — بلا حائط شرائح ولا صفوف مميزة
    }

    /* لا نصل هنا إلا إذا لم يوجد أي محتوى نشط إطلاقاً (لا قنوات ولا أعمال)،
       لأن الوضع التلقائي يبني بطاقة لكل قسم رئيسي له محتوى. */
    const wrap=document.getElementById('netflixStyleSliders');
    wrap.classList.remove('hidden');
    wrap.innerHTML='<div class="shs-hg-wrap"><div class="shs-hg-empty">'
      +'<div class="shs-hg-empty-ico"><svg xmlns="http://www.w3.org/2000/svg" width="46" height="46" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect width="20" height="15" x="2" y="7" rx="2"/><path d="m17 2-5 5-5-5"/></svg></div>'
      +'<div class="shs-hg-empty-title">لا يوجد محتوى متاح بعد</div>'
      +'<p class="shs-hg-empty-sub">أضف قنوات أو أفلاماً/مسلسلات من لوحة التحكم وستظهر هنا تلقائياً فور إضافتها — بلا أي إعداد إضافي.</p>'
      +'</div></div>';
    // استعادة الحالة من الرابط إن وُجدت (تصفّح مباشر لقسم/بحث/فيديو) — لا يكسره غياب المجموعات
    try{ await shsRestoreFromHash(); }catch(e){ console.error('restore:',e); }
...
function _prebuildFeaturedRowSlots(wrap){
  if(!HIDE_SUGGESTIONS) buildFeaturedRow(wrap,'suggestions','مقترحات قد تعجبك','✨');
...
async function loadFeaturedRows(wrap){
  ...
  try{
    ...
    // ── مقترحات قد تعجبك: خليط عشوائي يتغيّر كل زيارة ──
    // مكان الصف محجوز مسبقاً (Skeleton) عبر _prebuildFeaturedRowSlots — هنا فقط
    // نملأه أو نزيله إن لم يوجد محتوى كافٍ، دون أي بناء/إزاحة جديدة.
    if(!HIDE_SUGGESTIONS){
      const rowEl=document.querySelector('.netflix-slider-row[data-row-id="suggestions"]');
    ...
    // نفس المبدأ: الصفوف محجوزة مسبقاً، fillFeaturedRow نفسها تُزيل الصف
    // فوراً إن وصلت مصفوفة فارغة (راجع تعريفها) فلا حاجة لشرط هنا.
    if(!HIDE_MOST_WATCHED){
    ...
    // فشل جلب الصفوف المميزة لا يجب أن يترك Skeleton عالقاً للأبد —
    // نزيل أي صف محجوز لم يُملأ بعد فيختفي مكانه المحجوز بدل تجميد شكله.
    ['suggestions','most_watched_series','most_watched_channels'].forEach(id=>{
      const r=document.querySelector(`.netflix-slider-row[data-row-id="${id}"]`);
      if(r) r.remove();
    });
  }
}
...
async function openSeriesEpisodes(seriesId,seriesName,seriesPoster){
  ...
  try{
    ...
    /* Multi-Storage: حلقات على قرص غير متصل تُحجب مؤقتاً من الخادم — نقول
       ذلك صراحة بدل «لا توجد حلقات» التي توحي بأن المحتوى حُذف. */
    const _emptyP = empty.querySelector('p') || empty;
    if(_emptyP.dataset.orig === undefined) _emptyP.dataset.orig = _emptyP.textContent;
    _emptyP.textContent = (d && d.unavailable_count > 0 && !App.allEpisodes.length)
      ? 'هذا المحتوى غير متاح مؤقتاً — قرص التخزين الذي يحمله غير متصل الآن. حاول لاحقاً.'
      : _emptyP.dataset.orig;
...
/* تحميل قائم بالفعل — يمنع استدعاءين متزامنين (مثل: فتح البحث + استعادة قناة
   من رابط) من إطلاق نفس دفعات الطلبات مرتين معاً. سابقاً كان الحارس مجرد
   Boolean يُضبط فقط بعد اكتمال الحلقة الطويلة، فكل استدعاء يصل أثناء
   التحميل يراه false ويُعيد تشغيل الحلقة كاملة من الصفر — مضاعفةً حقيقية
   لعدد الطلبات على مكتبة كبيرة (مئات الفئات بعد استيراد Xtream)، وهذا ما
   كان يُصادف حدّ المعدّل العام لـapi.php (429) في استخدام طبيعي. */
let _shsAllContentPromise=null;
async function _shsEnsureAllContent(){
  if(_shsAllContentLoaded)return;
  if(!App.cats||!App.cats.length)return; // لا أقسام بعد
  if(_shsAllContentPromise)return _shsAllContentPromise;   // تحميل جارٍ بالفعل — ننتظره بدل تكراره

  _shsAllContentPromise = (async ()=>{
      ...
      for(let i=0; i<jobs.length; i+=BATCH){
        ...
        const res = await Promise.all(slice.map(j=>
          fetch(`api.php?action=${j.act}&category_id=${encodeURIComponent(j.id)}`)
            .then(r=>{
              // فشل حقيقي (429 أو غيره) يختلف عن فئة فارغة فعلاً — لا نخلطهما
              if(!r.ok){ hadFailure=true; return {type:j.type, items:[]}; }
              return r.json().then(d=>({type:j.type, items:(j.type==='series' ? (d.series||[]) : (d.channels||[]))}));
            })
            .catch(()=>{ hadFailure=true; return {type:j.type, items:[]}; })
        ));
        res.forEach(r=>{ r.items.forEach(k=>_shsAddContent(k, r.type)); });
        /* إمهال ثابت بين كل دفعة وأخرى (لا فقط بعد فشل): على مكتبة كبيرة
           بعد استيراد Xtream (مئات الأقسام)، كانت الدفعات تنطلق بلا أي
           تأخير فتستهلك حدّ معدّل الـAPI المشترك بالكامل خلال ثوانٍ —
           فيصطدم أي طلب آخر من نفس المتصفح (حتى all_content العادي) بـ429.
           40ms هنا لا تكاد تُلاحَظ على مكتبة صغيرة، وتحوّل مكتبة ضخمة إلى
           تدفّق منتظم بدل انفجار لحظي. نمهل أكثر بعد أي فشل كما كان. */
        await new Promise(r=>setTimeout(r, hadFailure ? 800 : 40));
      }
      /* نُعلن الاكتمال فقط إن نجحت كل الدفعات. فشل جزئي يعني نتائج بحث ناقصة
         فعلاً لا فئات فارغة — نترك العلَم false ليُعاد المحاولة عند أول
         استدعاء قادم بدل أن يبدو البحث مكتملاً وهو ناقص صامتاً إلى الأبد. */
      if(!hadFailure) _shsAllContentLoaded=true;
    }catch(e){ /* عند الفشل نكمل بالمحتوى المتاح، ونحاول من جديد عند الاستدعاء القادم */ }
    finally{ _shsAllContentPromise=null; }
  })();

  return _shsAllContentPromise;
}
...
/* ══════════════════════════════════════════════════════════════════════
   [SHS-DOCK] شريط الأقسام (Dock) — كل مجموعة (قسم أب) قائمة منسدلة
   ══════════════════════════════════════════════════════════════════════
   الرئيسية تعود كما كانت (حائط الشرائح). أما التنقّل بين الأقسام فيتم عبر
   شريط Dock علوي: كل قسم رئيسي (parent_id = null) له أقسام فرعية يظهر كزرّ
   قائمة منسدلة تُدرِج أبناءه؛ والقسم الذي بلا أبناء يظهر كزرّ مباشر يفتح
   محتواه. البنية (الأب/الابن) تُدار بالكامل من صفحة الإدارة (parent_id +
   السحب والإفلات في pages/categories.php)، فأي تغيير هناك ينعكس تلقائياً
   على هذا الشريط بلا أي تعديل يدوي على الواجهة.
   القائمة المنسدلة تُلحق بـ body وتُوضع بموضع ثابت تحت الزرّ، حتى لا تُقصّ
   داخل الشريط ذي التمرير الأفقي (overflow-x:auto).
   ══════════════════════════════════════════════════════════════════════ */
var _catDockGroups = {};   /* groupId -> node (له أبناء) */
...
function renderCategoryNavBar(){
  ...
  bar.innerHTML='';
  _catDockGroups={};
  var _navCats=shsNavCats();
  if(!_navCats.length){bar.style.display='none';return;}
  ...
  const tree=buildCategoryTree(_navCats);
  tree.roots.forEach(node=>{
    if(node.children && node.children.length){
      /* مجموعة لها أبناء → زرّ قائمة منسدلة */
      _catDockGroups[node.id]=node;
      const trg=document.createElement('button');
      trg.type='button';
      trg.className='cat-nav-btn cat-dock-trigger';
      trg.dataset.groupId=node.id;
      trg.setAttribute('aria-haspopup','true');
      trg.setAttribute('aria-expanded','false');
      trg.innerHTML='<span class="lcn">'+shsCatIconSVG(node.name)+'</span>'
        +'<span class="cat-dock-trigger-name"></span>'
        +'<span class="cat-dock-caret" aria-hidden="true"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"/></svg></span>';
      trg.querySelector('.cat-dock-trigger-name').textContent=node.name;
      trg.addEventListener('click',(e)=>{ e.stopPropagation(); catDockToggle(trg, node); });
      bar.appendChild(trg);
    }else{
      /* قسم بلا أبناء → زرّ مباشر */
      const btn=document.createElement('button');
      btn.type='button';
      btn.className='cat-nav-btn';
      btn.dataset.catId=node.id;
      btn.textContent=node.name;
      btn.addEventListener('click',()=>{ catDockClose(); openCategoryView(node.id,node.name); });
      bar.appendChild(btn);
    }
  });
...
/* الحاوية المشتركة للقائمة المنسدلة — تُنشأ مرة واحدة وتُلحق بـ body */
function _catDockEnsureFlyout(){
  var fly=document.getElementById('catDockFlyout');
  if(fly) return fly;
  fly=document.createElement('div');
  fly.id='catDockFlyout';
  fly.className='cat-dock-menu';
  fly.setAttribute('role','menu');
  fly.addEventListener('click',function(e){
    var it=e.target.closest('.cat-dock-item');
    if(!it||!fly.contains(it)) return;
    var id=it.dataset.catId, nm=it.dataset.catName||'';
    catDockClose();
    openCategoryView(id, nm);
  });
  document.body.appendChild(fly);
  /* مستمعات الإغلاق العامة — تُربط مرة واحدة فقط */
  if(!_catDockEnsureFlyout.__bound){
    _catDockEnsureFlyout.__bound=true;
    document.addEventListener('click',function(e){
      var open=document.querySelector('.cat-dock-trigger[aria-expanded="true"]');
      if(!open) return;
      if(e.target.closest('#catDockFlyout')||e.target.closest('.cat-dock-trigger')) return;
      catDockClose();
    });
    document.addEventListener('keydown',function(e){ if(e.key==='Escape') catDockClose(); });
    window.addEventListener('resize',function(){ catDockClose(); });
    /* إغلاق عند تمرير الصفحة أو تمرير الشريط الأفقي — أبسط وأنظف من إعادة الحساب */
    window.addEventListener('scroll',function(){ catDockClose(); }, true);
  }
  return fly;
}

function _catDockBuildItems(node){
  var items=[];
  /* لو كان للقسم الأب محتوى مباشر خاص به أيضاً، نضيف عنصراً لفتحه هو نفسه */
  var ownCount=(parseInt(node.channel_count||0,10)+parseInt(node.series_count||0,10));
  if(ownCount>0){
    items.push('<button type="button" class="cat-dock-item cat-dock-item--all" data-cat-id="'+esc(String(node.id))+'" data-cat-name="'+esc(node.name)+'">'
      +'<span class="lcn">'+shsCatIconSVG(node.name)+'</span>'
      +'<span class="cat-dock-item-name">'+esc(node.name)+' — الكل</span></button>');
  }
  (node.children||[]).forEach(function(c){
    var cnt=(parseInt(c.channel_count||0,10)+parseInt(c.series_count||0,10));
    items.push('<button type="button" class="cat-dock-item" data-cat-id="'+esc(String(c.id))+'" data-cat-name="'+esc(c.name)+'">'
      +'<span class="lcn">'+shsCatIconSVG(c.name)+'</span>'
      +'<span class="cat-dock-item-name">'+esc(c.name)+'</span>'
      +(cnt>0?'<span class="cat-dock-item-count">'+cnt+'</span>':'')
      +'</button>');
  });
  return items.join('');
}

function catDockToggle(trg, node){
  var wasOpen=(trg.getAttribute('aria-expanded')==='true');
  catDockClose();
  if(wasOpen) return;
  var fly=_catDockEnsureFlyout();
  fly.innerHTML=_catDockBuildItems(node);
  trg.setAttribute('aria-expanded','true');
  trg.classList.add('cat-dock-open');
  /* الموضع: تحت الزرّ، ومحاذاة RTL على الحافة اليمنى للزرّ */
  var r=trg.getBoundingClientRect();
  fly.style.display='block';
  fly.style.top=(r.bottom+6)+'px';
  var vw=document.documentElement.clientWidth;
  fly.style.right=Math.max(8,(vw-r.right))+'px';
  fly.style.left='auto';
  /* إن تجاوزت القائمة يسار الشاشة، نُحاذيها لليسار بدلاً من ذلك */
...
function catDockClose(){
  var fly=document.getElementById('catDockFlyout');
  if(fly){ fly.style.display='none'; fly.innerHTML=''; }
  document.querySelectorAll('.cat-dock-trigger[aria-expanded="true"]').forEach(function(t){
    t.setAttribute('aria-expanded','false');
    t.classList.remove('cat-dock-open');
  });
}
...
function shsRenderCatMenu(){
  ...
  if(cnt)cnt.textContent=_menuCats.length+' قسم متاح';
}
...
/* ════ PLAYER STATE ════ */
const PL={hls:null,dash:null,flv:null,vol:1,muted:false,idle:null,subtitleOn:false,episodeId:0,episodeTitle:'',subtitleLookup:false,epPanelOpen:false,m3uPanelOpen:false,m3uEntries:[],m3uIdx:-1,userPaused:false,backupUrl:'',usedBackup:false,audioUrl:'',audioDelay:0,serverMuxAudio:false,externalAudio:null,audioHls:null,audioActive:false,audioSyncTimer:null};
...
function openPlayerEpisode(idx){
  ...
  /* مرجع الحلقة لزر الترجمة: بدونه لا يعرف المشغّل عن أي حلقة يسأل
     خدمة الترجمة، فيبقى الزر بلا فائدة حين لا يوجد ملف محفوظ. */
...
var _prevScreen={ep:false,home:false,search:false,category:false,group:false,groupId:null,groupScrollY:0};
...
function _openOverlay(url,subUrl){
  ...
  _prevScreen.group=!document.getElementById('homeGroupSection').classList.contains('hidden');
  _prevScreen.groupId=_prevScreen.group&&App.currentHomeGroup?App.currentHomeGroup.id:null;
  _prevScreen.groupScrollY=_prevScreen.group?(window.scrollY||0):0;
...
function closePlayer(){
  ...
  document.getElementById('homeGroupSection').classList.toggle('hidden',!_prevScreen.group);
  if(_prevScreen.group && App.currentHomeGroup){
    window.scrollTo({top:_prevScreen.groupScrollY||0,behavior:'instant'});
  }
}
  ...
  if(fmt==='hls'){
    ...
    if(typeof Hls!=='undefined'&&Hls.isSupported()){
      PL.hls=new Hls({
        ...
        /* ابدأ البث المعاد فور وصول أول مقطع صالح. الإعداد السابق كان
           ينتظر ثلاثة مقاطع ويبتعد 8 ثوانٍ عن الحافة الحية، فيبدو للمشاهد
           أن القناة متأخرة 12–15 ثانية رغم أن الوسيط جاهز. نُبقي هامشاً
           صغيراً للاستقرار، ثم يبني المشغّل المخزن أثناء التشغيل. */
        maxBufferLength: PL._hlsLocalStream ? 12 : 15,
        maxMaxBufferLength: PL._hlsLocalStream ? 24 : 30,
        maxBufferSize: 40 * 1000 * 1000,
        backBufferLength: PL._hlsLocalStream ? 15 : 20,
        maxBufferHole: 0.8,

        liveSyncDuration: PL._hlsLocalStream ? 2 : 2.5,
        liveMaxLatencyDuration: PL._hlsLocalStream ? 8 : 12,
...
/* ════ SUBTITLE TOGGLE ════ */
/*
 * زر الترجمة كان ينتهي عند «لا تتوفر ترجمة» متى غاب ملفٌ مرفوع من اللوحة —
 * وهو حال معظم الأفلام. صار الآن: اقرأ المحفوظ أولاً (بلا أي طلب خارجي)،
 * فإن لم يوجد اعرض قائمة الاختيارات ليقرّر المشاهد بنفسه؛ فالاختيار الأول
 * آلياً كثيراً ما يكون لنسخة مختلفة فتظهر الترجمة غير متزامنة.
 * الملف المختار يُحفظ على الخادم مرة واحدة، فلا يتكرر الطلب الخارجي.
 */
async function shsEnableEpisodeSubtitle(video,url){
  const ep=App.allEpisodes&&App.allEpisodes[App.currentEpisodeIdx];
  if(ep)ep.subtitle_url=url;
  PL.subtitleOn=true;
  await _loadSubtitle(video,url);
  ...
  if(btn){btn.style.opacity='1';btn.style.color='#ff4d57';btn.classList.add('sub-active');}
  toast('✓ الترجمة مفعّلة');
  return true;
}
...
function shsOpenSubtitlePicker(episodeId,video){
  ...
  fetch('api.php?action=subtitle_options&episode_id='+encodeURIComponent(episodeId),{cache:'no-store'}).then(r=>r.json()).then(d=>{
    ...
    if(!options.length){toast((d&&d.message)||'لا تتوفر ترجمة');return;}
    const prev=document.getElementById('shsSubtitlePicker');if(prev)prev.remove();
    ...
    const closeIt=()=>{modal.remove();document.removeEventListener('keydown',onKey);};
    const onKey=e=>{if(e.key==='Escape'){e.stopPropagation();closeIt();}};
    modal.querySelector('[data-close]').onclick=closeIt;
    modal.onclick=e=>{if(e.target===modal)closeIt();};
    document.addEventListener('keydown',onKey);
    const list=modal.querySelector('[data-list]');
    options.forEach(opt=>{
      const b=document.createElement('button');b.type='button';
      b.style.cssText='text-align:right;color:#fff;background:#20232a;border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:12px;cursor:pointer;font:inherit';
      b.innerHTML='<b>'+shsSubtitleEsc(opt.title)+'</b><small style="display:block;color:#b8bfcc;margin-top:5px">'+shsSubtitleEsc(opt.language||'')+' '+shsSubtitleEsc(opt.year||'')+' · '+Number(opt.downloads||0).toLocaleString()+' تحميل</small>';
      b.onclick=()=>{
        b.disabled=true;b.style.opacity='.6';b.querySelector('b').textContent='جارٍ تنزيل الترجمة…';
        fetch('api.php?action=subtitle_download',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'episode_id='+encodeURIComponent(episodeId)+'&file_id='+encodeURIComponent(opt.file_id)})
          .then(r=>r.json())
          .then(async x=>{
            if(!x||!x.success||!x.subtitle_url){toast((x&&x.error)||'تعذّر تنزيل الترجمة');b.disabled=false;b.style.opacity='1';return;}
            closeIt();
            await shsEnableEpisodeSubtitle(video,String(x.subtitle_url));
          })
          .catch(()=>{toast('تعذّر تنزيل الترجمة');b.disabled=false;b.style.opacity='1';});
      };
      list.appendChild(b);
    });
...
function toggleSubtitle(){
  ...
  if(btn){
    ...
    // صنف الحالة كان يُضاف عند الإغلاق فقط فيبقى الزر «مفعّلاً» شكلاً بعد الإيقاف
    btn.classList.toggle('sub-active', PL.subtitleOn);
  }
...
(function(){
  window._goBack=function(){
    ...
    var _sec=function(id){var el=document.getElementById(id);return el && !el.classList.contains('hidden');};
    if(_sec('epSection')){backFromEpisodesToHome(true);return;}
    if(_sec('searchViewSection')){clearSearchAndGoHome();return;}
    if(_sec('categoryViewSection')){closeCategoryView(true);return;}
    if(_sec('homeGroupSection')){shsCloseGroupView(true);return;}
  };
  /* إيماءة الحافة: كانت تستدعي _goBack مباشرة فتُغلق الشاشة دون أن تستهلك
     إدخالها من سجل المتصفح. فيبقى في السجل إدخال بلا شاشة، وأول رجوع
     لاحق من النظام يُغلق المستوى التالي — أي قفزتان بدل واحدة، وهذا ما
     كان يُعيد الهاتف إلى الرئيسية مباشرة. الآن تمرّ عبر history.back()
     حين توجد إدخالة، فيبقى مسار واحد: popstate ← _goBack.
     وعدّاد popstate يمنع الازدواج: إن نفّذ المتصفح رجوعه أثناء الإيماءة
     نفسها (إيماءة النظام) لا نضيف رجوعاً ثانياً. */
  var gsx=0,gsy=0,gActive=false,gPop=0;
  var EDGE=0.18,MIN_X=65,MAX_Y=65;
  window.__shsPopCount=window.__shsPopCount||0;
  window.addEventListener('popstate',function(){window.__shsPopCount++;});
  document.addEventListener('touchstart',function(e){var t=e.changedTouches[0];gsx=t.screenX;gsy=t.screenY;gPop=window.__shsPopCount;var w=window.innerWidth;gActive=(gsx<w*EDGE)||(gsx>w*(1-EDGE));},{passive:true});
  document.addEventListener('touchcancel',function(){gActive=false;},{passive:true});
  document.addEventListener('touchend',function(e){
    if(!gActive)return;
    var t=e.changedTouches[0];var dx=t.screenX-gsx,dy=Math.abs(t.screenY-gsy);gActive=false;
    if(Math.abs(dx)<MIN_X||dy>MAX_Y)return;
    if(window.__shsPopCount!==gPop)return;           // المتصفح رجع بالفعل خلال هذه الإيماءة
    var st=null; try{st=history.state;}catch(_){}
    if(st && (st.shsView||st.depth||st.player)){ try{history.back();return;}catch(_){} }
    window._goBack();
  },{passive:true});
  ...
  window.openSeriesEpisodes=function(){
    var _vis=function(id){var el=document.getElementById(id);return !!(el&&!el.classList.contains('hidden'));};
    /* الأصل يُسجَّل فقط عند الدخول من شاشة أخرى؛ الانتقال من عمل إلى
       عمل داخل صفحة الحلقات يُبقي الأصل الأول وإدخال السجل الواحد. */
    if(!_vis('epSection')){
      App.__epOrigin = _vis('searchViewSection') ? 'search' : (_vis('categoryViewSection') ? 'category' : 'home');
      App.__epScrollY = window.scrollY||0;
    }
    var lbl=document.getElementById('epBackLabel');
    if(lbl){
      lbl.textContent = App.__epOrigin==='category' && App.currentCategoryView ? ('رجوع إلى ' + (App.currentCategoryView.name||'القسم'))
                      : App.__epOrigin==='search' ? 'رجوع إلى البحث' : 'الرئيسية';
    }
    if(!(history.state && history.state.depth==='episodes')) history.pushState({depth:'episodes'},'');
    return _origOpenSeries.apply(this,arguments);
  };
})();
...
(function(){
    ...
    window.addEventListener('unhandledrejection', function(ev){
        try {
            ...
            var now = Date.now();
            if (now - _lastNetToastAt > 4000 && typeof toast === 'function' && document.getElementById('toastContainer')) {
                _lastNetToastAt = now;
...
/* ══════════════════════════════════════════════════════════════════════
   [SHS-TREE] Dynamic Content Tree — بنية شجرة الأقسام الديناميكية للرئيسية
   ══════════════════════════════════════════════════════════════════════
   الفكرة: كل ما نحتاجه لبناء الشجرة كاملة (الأنواع الرئيسية وكل الفروع)
   موجود فعلاً في استجابة api.php?action=all_content الخفيفة (اسم/parent_id/
   عدد قنوات/عدد أعمال لكل قسم فقط، بلا أي عناصر محتوى) — نفس الطلب الذي
   كانت الرئيسية القديمة تستخدمه أصلاً. لذلك نبني الشجرة محلياً من parent_id
   (الحقل موجود في قاعدة البيانات ومربوط بالفعل في لوحة التحكم، ولم يكن
   يُستخدم في الواجهة العامة أبداً) دون أي طلب شبكة إضافي أو تغيير API.
   التنقّل بين مستويات الشجرة (نوع رئيسي ← أقسام فرعية) فوري ومن الذاكرة.
   محتوى القسم الفعلي (قنوات/مسلسلات) لا يُجلب إلا عند وصول المستخدم فعلاً
   لقسم ورقي (بلا أبناء) عبر openCategoryView الموجودة والمُختبرة أصلاً —
   بكامل تخزينها المؤقت وحالات التحميل/الفراغ/الخطأ الحالية، بلا إعادة بناء.
   ══════════════════════════════════════════════════════════════════════ */

/* بناء شجرة الأقسام من القائمة المسطّحة عبر parent_id.
   أي قسم أبوه غير موجود ضمن القائمة (أو ليس له أب) يُعتبر "نوعاً رئيسياً"
   (مستوى الجذر) — ديناميكياً بالكامل، بلا أي اسم/نوع مكتوب يدوياً. */
/* ══════════════════════════════════════════════════════════════════════
   [SHS-XT] الأقسام المستوردة (Xtream / M3U) لا تظهر في تنقّل index وحدها
   ──────────────────────────────────────────────────────────────────────
   استيراد حساب Xtream واحد يُنشئ عشرات الأقسام الجذرية (قنوات + أفلام +
   مسلسلات لكل تصنيف في اللوحة)، وقائمة M3U واحدة تُنشئ قسماً لكل
   group-title فيها. قبل هذا الحدّ كانت كلها تنزل دفعةً واحدة
   على الصفحة الرئيسية وعلى شريط Dock وقائمة الأقسام، فتُغرِق ما رتّبه
   المدير بيده تحت مئات البطاقات.

   القاعدة الآن: قسم Xtream يظهر في التنقّل فقط إذا وضعه المدير داخل
   مجموعة من مجموعات الصفحة الرئيسية (بما في ذلك عبر اختيار قسم أب).
   الأقسام العادية لا يمسّها هذا الحدّ إطلاقاً.

   مهم: لا نُرشِّح App.cats نفسها — فبطاقات المجموعات وفتح القسم وعرض
   المحتوى كلها تقرأ منها بالمعرّف. نرشّح التنقّل وحده.
   ══════════════════════════════════════════════════════════════════════ */
function shsHgAllowedXtreamIds(){
  var allowed={};
  var defs=(App.homeGroupsConfigured && Array.isArray(App.homeGroupDefs)) ? App.homeGroupDefs : [];
  if(!defs.length) return allowed;

  var cats=Array.isArray(App.cats)?App.cats:[];
  var children={};
  cats.forEach(function(c){
    var pid=String(parseInt(c.parent_id||0,10)||0);
    if(pid!=='0'){ (children[pid]=children[pid]||[]).push(String(c.id)); }
  });

  defs.forEach(function(def){
    (Array.isArray(def.category_ids)?def.category_ids:[]).forEach(function(rootId){
      var stack=[String(rootId)];
      while(stack.length){
        var cur=String(stack.pop());
        if(allowed[cur]) continue;
        allowed[cur]=1;
        (children[cur]||[]).forEach(function(ch){ if(!allowed[ch]) stack.push(ch); });
      }
    });
  });
  return allowed;
...
/* «مستورد» = من حساب Xtream أو من قائمة M3U. الحكم واحد: كلاهما يُنشئ
   عشرات الأقسام بضغطة واحدة، والمدير لم يختر أياً منها للعرض.
   is_xtream يبقى مقروءاً للتوافق مع استجابة مخزّنة مؤقتاً من إصدار أقدم. */
function shsCatIsImported(c){
  return !!(parseInt((c&&c.is_imported)||0,10) || parseInt((c&&c.is_xtream)||0,10));
}

function shsNavCats(){
  var cats=Array.isArray(App.cats)?App.cats:[];
  var hasImported=cats.some(shsCatIsImported);
  if(!hasImported) return cats;                     // لا شيء يُرشَّح
  var allowed=shsHgAllowedXtreamIds();
  return cats.filter(function(c){
    if(!shsCatIsImported(c)) return true;           // قسم عادي — يظهر دائماً
    return !!allowed[String(c.id)];                 // قسم مستورد — بإذن مجموعة
  });
}
...
/* ══════════════════════════════════════════════════════════════════════
   [SHS-HG] مجموعات الصفحة الرئيسية على index — بطاقات منظّمة بأغلفة، بلا
   عرض كل المحتوى مرة واحدة وبلا عشوائية. البيانات من api.php?action=home_groups
   (يديرها المدير). النقر على مجموعة: قسم واحد ⇐ يفتح محتواه؛ عدة أقسام ⇐
   يعرض أقسامها كبطاقات ثم المحتوى. لا يُحمَّل أي محتوى فعلي قبل النقر.
   ══════════════════════════════════════════════════════════════════════ */
function _shsHgApplyCovers(scope){
  (scope||document).querySelectorAll('.shs-hg-card[data-cover]').forEach(function(el){
    var cover=el.getAttribute('data-cover')||'';
    if(cover) shsApplyHgCover(el, cover);
  });
}
function _shsHgCardHTML(o){
  var color=/^#[0-9a-fA-F]{3,8}$/.test(o.color||'')?o.color:'#38bdf8';
  return '<button type="button" class="shs-hg-card'+(o.isSub?' shs-hg-card--sub':'')+'" data-hg-id="'+esc(String(o.id))+'" data-cover="'+esc(o.cover||'')+'" data-hg-kind="'+esc(o.kind||'auto')+'" data-hg-idx="'+(parseInt(o.idx,10)||0)+'" style="--hg-color:'+esc(color)+'">'
    +'<span class="shs-hg-art" aria-hidden="true"></span>'
    +'<span class="shs-hg-glow" aria-hidden="true"></span>'
    +'<span class="shs-hg-ico" aria-hidden="true">'+shsCatIconSVG(o.name,'1.6em')+'</span>'
    +'<span class="shs-hg-body">'
      +'<strong class="shs-hg-name">'+esc(o.name||'')+'</strong>'
      +(o.meta?'<span class="shs-hg-meta">'+esc(o.meta)+'</span>':'')
    +'</span>'
    +'<span class="shs-hg-arrow" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg></span>'
    +'</button>';
}
function shsRenderHomeGroups(groups){
  var hgSec=document.getElementById('homeGroupSection'); if(hgSec) hgSec.classList.add('hidden');
  var cv=document.getElementById('categoryViewSection'); if(cv) cv.classList.add('hidden');
  var wrap=document.getElementById('netflixStyleSliders'); if(!wrap) return;
  wrap.classList.remove('hidden');
  var cards=groups.map(function(g,i){
    var cats=g.category_count||0, items=g.item_count||0;
    var meta=(cats>1?cats+' قسم • ':'')+(items?Number(items).toLocaleString('ar-EG')+' عنصر':'');
    return _shsHgCardHTML({id:g.id,name:g.name,meta:meta,color:g.color,cover:g.cover,kind:g.content_type,idx:i});
  }).join('');
  wrap.innerHTML='<div class="shs-hg-wrap">'
    +'<div class="shs-hg-head"><h2 class="shs-hg-h2">اختر نوع المحتوى</h2><p class="shs-hg-p">تُحدَّث الأقسام والأعداد تلقائياً من لوحة الإدارة.</p></div>'
    +'<div class="shs-hg-grid" id="homeGroupsGrid">'+cards+'</div></div>';
  _shsHgApplyCovers(wrap);
  shsLoadHgArtwork(wrap);
  var grid=document.getElementById('homeGroupsGrid');
  if(grid && !grid.__bound){
    grid.__bound=true;
    grid.addEventListener('click',function(e){ var b=e.target.closest('.shs-hg-card'); if(b&&grid.contains(b)) openHomeGroup(b.dataset.hgId); });
  }
}
function openHomeGroup(id){
  var g=(App.homeGroups||[]).find(function(x){return String(x.id)===String(id);}); if(!g) return;
  var ids=g.category_ids||[];
  if(ids.length===1){ openCategoryFromGroup(ids[0], g.name); }
  else if(ids.length>1){ shsRenderGroupView(g); }
}
function shsRenderGroupView(g, opts){
  var wrap=document.getElementById('netflixStyleSliders'); if(wrap) wrap.classList.add('hidden');
  var hw=document.getElementById('heroWelcome'); if(hw) hw.classList.add('hidden');
  var cv=document.getElementById('categoryViewSection'); if(cv) cv.classList.add('hidden');
  var sec=document.getElementById('homeGroupSection'); if(!sec) return;
  sec.classList.remove('hidden');
  App.currentHomeGroup=g;
  /* التحديث التلقائي يعيد رسم المجموعة نفسها؛ لا نضيف عندها خطوة رجوع جديدة. */
  if(!(opts&&opts.restore)) shsPushView('group');
  var titleEl=document.getElementById('homeGroupTitle'); if(titleEl) titleEl.textContent=g.name||'المجموعة';
  var byId={}; (App.cats||[]).forEach(function(c){ byId[c.id]=c; });
  var ids=g.category_ids||[];
  var cards=ids.map(function(cid,i){
    var c=byId[cid]; if(!c) return '';
    var ch=parseInt(c.channel_count||0,10), sr=parseInt(c.series_count||0,10), tot=ch+sr;
    var meta=tot?Number(tot).toLocaleString('ar-EG')+' عنصر':'';
    var cov=(ch>=sr ? (c.cover_channel||c.cover_series) : (c.cover_series||c.cover_channel))||'';
    var kind=(ch>=sr)?'live':'movie';
    return _shsHgCardHTML({id:'cat-'+cid,name:c.name,meta:meta,color:g.color,cover:cov,kind:kind,idx:i});
  }).join('');
  var grid=document.getElementById('homeGroupGrid');
  if(grid){
    grid.innerHTML=cards || '<div style="grid-column:1/-1;text-align:center;padding:40px;color:var(--text-muted)">لا توجد أقسام في هذه المجموعة.</div>';
    _shsHgApplyCovers(grid);
    shsLoadHgArtwork(grid);
    if(!grid.__bound){
      grid.__bound=true;
      grid.addEventListener('click',function(e){
        var b=e.target.closest('.shs-hg-card'); if(!b||!grid.contains(b)) return;
        var raw=b.dataset.hgId||'';
        if(raw.indexOf('cat-')===0){
          var cid=raw.slice(4);
          var c=(App.cats||[]).find(function(x){return String(x.id)===String(cid);});
          openCategoryFromGroup(cid, c?c.name:'');
        }
      });
    }
  }
  /* العودة من قسم تُعيد المستخدم إلى موضعه في القائمة، لا إلى أعلاها —
     في مجموعة من عشرات الأقسام كان يضطر للتمرير من جديد بعد كل رجوع. */
  /* 'instant' صراحةً: الصفحة تفرض scroll-behavior:smooth، فيتحوّل الرجوع
     إلى تمرير متحرّك عبر آلاف البكسلات — يبدو كأن الصفحة «تتحرك وحدها». */
  if(opts && typeof opts.scrollY==='number'){ window.scrollTo({top:opts.scrollY, behavior:'instant'}); }
  else { window.scrollTo({top:0,behavior:'smooth'}); }
}
function openCategoryFromGroup(catId, name){
  var sec=document.getElementById('homeGroupSection');
  /* الأصل «مجموعة» فقط إن كنا فعلاً داخل شاشة المجموعة. المجموعة ذات القسم
     الواحد تفتح قسمها من الرئيسية مباشرة، فأصلها الرئيسية لا المجموعة. */
  var fromView = !!(sec && !sec.classList.contains('hidden') && App.currentHomeGroup);
  App.__pendingCatOrigin = fromView ? {type:'group', g:App.currentHomeGroup, scrollY:(window.scrollY||0)} : null;
  if(sec) sec.classList.add('hidden');
  openCategoryView(catId, name);
}
function shsCloseGroupView(fromHistory){
  /* من زر الواجهة: نستهلك إدخال السجل فيتولّى popstate الإغلاق — فلا يتراكم */
  if(!fromHistory && shsViewIsPushed('group')){ try{ history.back(); return; }catch(e){} }
  App.currentHomeGroup=null;
  var sec=document.getElementById('homeGroupSection'); if(sec) sec.classList.add('hidden');
  var cv=document.getElementById('categoryViewSection'); if(cv) cv.classList.add('hidden');
  var wrap=document.getElementById('netflixStyleSliders'); if(wrap) wrap.classList.remove('hidden');
  var hw=document.getElementById('heroWelcome'); if(hw) hw.classList.remove('hidden');
  try{ setActiveCatNavBtn(null); }catch(e){}
  window.scrollTo({top:0,behavior:'smooth'});
}


/* ══════════════════════════════════════════════════════════════════════
   [SHS-HG] حساب بطاقات الصفحة الرئيسية في المتصفح من App.cats (مبدأ roo)
   • الوضع التلقائي: بطاقة لكل قسم رئيسي له محتوى (هو أو فروعه) — أي قسم
     جديد يظهر فوراً بلا أي خطوة من المدير.
   • الوضع اليدوي (بعد تعريف مجموعات من اللوحة): المجموعات المعرّفة فقط.
   الأغلفة والأعداد كلها من نفس استجابة all_content، بلا عشوائية.
   ══════════════════════════════════════════════════════════════════════ */
function shsComputeHomeGroups(){
  var cats = Array.isArray(App.cats) ? App.cats : [];
  if(!cats.length) return [];

  var byId = {}, children = {};
  cats.forEach(function(c){ byId[String(c.id)] = c; children[String(c.id)] = []; });
  cats.forEach(function(c){
    var pid = String(parseInt(c.parent_id||0,10)||0);
    if(pid !== '0' && children[pid]) children[pid].push(String(c.id));
  });
  ...
  function subtree(rootId){
    var out=[], stack=[String(rootId)], seen={};
    while(stack.length){
      var cur=String(stack.pop());
      if(seen[cur] || !byId[cur]) continue;
      seen[cur]=1; out.push(cur);
      (children[cur]||[]).forEach(function(ch){ if(!seen[ch]) stack.push(ch); });
    }
    return out;
  }
  function tally(ids){
    ...
    ids.forEach(function(id){
      ...
      var a=parseInt(c.channel_count||0,10)||0, b=parseInt(c.series_count||0,10)||0;
      ch+=a; sr+=b;
      if(a+b>0) nav.push(parseInt(id,10));
    });
    return {channels:ch, series:sr, items:ch+sr, nav:nav};
  }
  function coverOf(ids, type){
    var first='', firstAlt='';
    for(var i=0;i<ids.length;i++){
      var c=byId[ids[i]]; if(!c) continue;
      var poster=(c.cover_series||'').toString(), logo=(c.cover_channel||'').toString();
      var pref = (type==='live') ? logo : poster;
      var alt  = (type==='live') ? poster : logo;
      if(pref && !first) first=pref;
      if(alt && !firstAlt) firstAlt=alt;
      if(first) break;
    }
    return first || firstAlt || '';
  }

  var out=[];

  // ── الوضع اليدوي: المجموعات المعرّفة من اللوحة فقط ──
  if(App.homeGroupsConfigured && Array.isArray(App.homeGroupDefs) && App.homeGroupDefs.length){
    App.homeGroupDefs.forEach(function(def){
      var sel=(Array.isArray(def.category_ids)?def.category_ids:[]).map(String).filter(function(id){return !!byId[id];});
      if(!sel.length) return;
      var ids=[], seen={};
      sel.forEach(function(id){ subtree(id).forEach(function(x){ if(!seen[x]){ seen[x]=1; ids.push(x); } }); });
      var t=tally(ids);
      var type=def.content_type||'auto';
      if(type==='auto') type = (t.channels>=t.series) ? 'live' : 'movie';
      out.push({
        id: parseInt(def.id,10)||0,
        name: def.name || 'مجموعة',
        description: def.description || '',
        color: def.accent_color || '#38bdf8',
        content_type: type,
        category_ids: t.nav.length ? t.nav : sel.map(function(x){return parseInt(x,10);}),
        cover: coverOf(ids, type),
        category_count: (t.nav.length || sel.length),
        item_count: t.items
      });
    });
    return out;
  ...
  // ── الوضع التلقائي: بطاقة لكل قسم رئيسي له محتوى ──
  cats.forEach(function(c){
    var pid=String(parseInt(c.parent_id||0,10)||0);
    if(pid !== '0' && byId[pid]) return;           // ليس قسماً رئيسياً
    /* الأقسام المستوردة (Xtream / M3U) خارج التقسيم التلقائي: استيراد
       واحد يُنشئ عشرات الأقسام الجذرية فتغرق الصفحة. تدخل فقط عبر
       مجموعة يحدّدها المدير. */
    if(shsCatIsImported(c)) return;
    var ids=subtree(c.id);
    var t=tally(ids);
    if(t.items<=0) return;                          // لا بطاقات فارغة
    var type=(t.channels>=t.series) ? 'live' : 'movie';
    out.push({
      ...
      content_type: type,
      category_ids: t.nav.length ? t.nav : [parseInt(c.id,10)],
      cover: coverOf(ids, type),
      category_count: t.nav.length || 1,
      item_count: t.items,
      auto: true
    });
  });
...
/* ══════════════════════════════════════════════════════════════════════
   [SHS-HG] أغلفة TMDB لبطاقات الصفحة الرئيسية حسب نوع المحتوى (مبدأ roo)
   • الأولوية دائماً لصورة المحتوى الحقيقية (بوستر عمل أو شعار قناة).
   • البطاقات التي لا صورة لها تأخذ غلافاً من TMDB حسب نوعها عبر وكيل
     مغلق المسارات (index.php?tmdb_proxy=portal&kind=…)، بطلب واحد لكل نوع
     وكاش ٦ ساعات في المتصفح — فلا استنزاف لمفتاح TMDB ولا عشوائية لكل زائر.
   ══════════════════════════════════════════════════════════════════════ */
var SHS_HG_ART_KEY = 'shs_hg_tmdb_art_v1_' + ((window.matchMedia && window.matchMedia('(max-width:600px)').matches) ? 'm' : 'd');

function shsHgArtCacheRead(){
  try{
    var v = JSON.parse(localStorage.getItem(SHS_HG_ART_KEY) || 'null');
    if(v && v.items && (Date.now() - (parseInt(v.savedAt||0,10)||0) < 21600000)) return v;
  }catch(e){}
  return {savedAt:0, items:{}};
}
function shsHgArtCacheWrite(cache){
  try{ cache.savedAt = Date.now(); localStorage.setItem(SHS_HG_ART_KEY, JSON.stringify(cache)); }catch(e){}
}
function shsHgArtUrls(results){
  var urls=[], seen={};
  ...
  (Array.isArray(results)?results:[]).forEach(function(item){
    var path = (item && typeof item.backdrop_path==='string' && item.backdrop_path)
      ? item.backdrop_path
      : ((item && typeof item.poster_path==='string') ? item.poster_path : '');
    if(!/^\/[A-Za-z0-9._-]+$/.test(path) || seen[path]) return;
    seen[path]=1;
    urls.push('https://image.tmdb.org/t/p/' + ((item.backdrop_path && !compact) ? 'w780' : 'w500') + path);
  });
  return urls;
}
function shsApplyHgCover(el, url){
  if(!el || !url) return;
  var art = el.querySelector('.shs-hg-art');
  var css = 'url("' + String(url).replace(/"/g,'%22') + '")';
  if(art){ art.style.backgroundImage = css; }
  else { el.style.backgroundImage = css; }
  el.classList.add('has-cover');      // يمنع طلب TMDB لهذه البطاقة
  el.classList.add('has-artwork');    // يفعّل تدرّج القراءة فوق البوستر
}
function shsLoadHgArtwork(scope){
  var root = scope || document;
  var cards = Array.prototype.slice.call(root.querySelectorAll('.shs-hg-card[data-hg-kind]:not(.has-cover)'));
  if(!cards.length) return;

  var cache = shsHgArtCacheRead();
  var need = {};
  cards.forEach(function(card){
    var kind = card.getAttribute('data-hg-kind') || 'auto';
    var idx  = parseInt(card.getAttribute('data-hg-idx')||'0',10) || 0;
    var entry = cache.items[kind];
    if(entry && Array.isArray(entry.urls) && entry.urls.length){
      shsApplyHgCover(card, entry.urls[idx % entry.urls.length]);
    } else {
      (need[kind] = need[kind] || []).push({card:card, idx:idx});
    }
  });

  var kinds = Object.keys(need);
  if(!kinds.length) return;
  /* تهدئة بعد رفض/تجاوز حدّ: لا نعيد المحاولة قبل انقضاء المهلة */
  try{ if(Date.now() < (parseInt(sessionStorage.getItem('shs_hg_art_retry_after')||'0',10)||0)) return; }catch(e){}

  Promise.all(kinds.map(function(kind){
    return fetch('index.php?tmdb_proxy=portal&kind=' + encodeURIComponent(kind))
      .then(function(r){
        if(r.status === 429){ try{ sessionStorage.setItem('shs_hg_art_retry_after', String(Date.now()+600000)); }catch(e){} return null; }
        return r.ok ? r.json() : null;
      })
      .then(function(d){
        var urls = shsHgArtUrls(d && d.results);
        if(!urls.length) return;
        cache.items[kind] = {urls:urls};
        need[kind].forEach(function(o){ shsApplyHgCover(o.card, urls[o.idx % urls.length]); });
      })
      .catch(function(){});
  })).then(function(){ shsHgArtCacheWrite(cache); });
}


/* ══════════════════════════════════════════════════════════════════════
   [SHS-NAV] سجل تصفّح داخلي: كل شاشة داخلية تُسجّل إدخالاً في المتصفح، فزر
   الرجوع (أو إيماءة الحافة على الهاتف) يُغلق الشاشة ويعود للرئيسية بدل أن
   يخرج من الموقع مباشرةً. إغلاق الشاشة من زر الواجهة يستهلك نفس الإدخال
   حتى لا يتراكم سجل وهمي.
   ══════════════════════════════════════════════════════════════════════ */
function shsPushView(name){
  try{
    if(history.state && history.state.shsView === name) return;
    history.pushState({shsView:name}, '');
  }catch(e){}
}
function shsViewIsPushed(name){
  try{ return !!(history.state && history.state.shsView === name); }catch(e){ return false; }
}


/* ══════════════════════════════════════════════════════════════════════
   [SHS-NAV-STEP] الرجوع مرحلة مرحلة
   ──────────────────────────────────────────────────────────────────────
   الترتيب الطبيعي: الرئيسية ← مجموعة ← قسم ← حلقات ← مشغّل.
   قبل هذا كان «إغلاق القسم» يعود للرئيسية دائماً لأنه لا يعرف من أين
   فُتح؛ فمن فتح قسماً من داخل مجموعة ثم رجع، قفز فوق المجموعة كلها.
   والأسوأ أن سجل المتصفح بقي فيه إدخال المجموعة بلا شاشة تقابله، فكان
   الضغط التالي «يضيع» أو يُغلق مستوى إضافياً.

   الآن كل شاشة تحفظ أصلها لحظة فتحها، والرجوع يعيد الأصل نفسه وموضع
   التمرير فيه — بنفس المسار سواء جاء من زر الواجهة أو زر المتصفح أو
   إيماءة الحافة في الهاتف (كلها تمرّ عبر popstate ثم _goBack).
   ══════════════════════════════════════════════════════════════════════ */
function _shsSecVisible(id){ var el=document.getElementById(id); return !!(el && !el.classList.contains('hidden')); }

function _shsSetCatBackLabel(){
  var el=document.getElementById('catBackLabel'); if(!el) return;
  var o=App.catOrigin;
  el.textContent = (o && o.type==='group' && o.g) ? ('رجوع إلى ' + (o.g.name||'المجموعة')) : 'الرئيسية';
}

if(typeof openCategoryView==='function' && !openCategoryView.__shsNavHook){
  var _shsOCVNav = openCategoryView;
  openCategoryView = function(catId, catName){
    /* يُحسب الأصل قبل أن تُخفي الدالة الداخلية الشاشات */
    if(App.__pendingCatOrigin){
      App.catOrigin = App.__pendingCatOrigin;                      // من بطاقة داخل المجموعة
    } else if(_shsSecVisible('homeGroupSection') && App.currentHomeGroup){
      App.catOrigin = {type:'group', g:App.currentHomeGroup, scrollY:(window.scrollY||0)}; // من الشريط أثناء عرض مجموعة
    } else if(_shsSecVisible('categoryViewSection')){
      /* انتقال جانبي من قسم إلى قسم: نفس المستوى ونفس إدخال السجل،
         فيبقى الأصل كما هو */
    } else {
      App.catOrigin = null;                                        // من الرئيسية
    }
    App.__pendingCatOrigin = null;
    var hg=document.getElementById('homeGroupSection'); if(hg) hg.classList.add('hidden');
    shsPushView('category');
    _shsSetCatBackLabel();
    return _shsOCVNav.apply(this, arguments);
  };
  openCategoryView.__shsFast = true;
  openCategoryView.__shsNavHook = true;
}
if(typeof closeCategoryView==='function' && !closeCategoryView.__shsNavHook){
  var _shsCCVNav = closeCategoryView;
  closeCategoryView = function(fromHistory){
    /* من زر الواجهة: نستهلك إدخال السجل، وpopstate يعود إلى هنا بـ fromHistory */
    if(!fromHistory && shsViewIsPushed('category')){ try{ history.back(); return; }catch(e){} }
    var origin = App.catOrigin; App.catOrigin = null;
    if(origin && origin.type==='group' && origin.g){
      App.currentCategoryView=null;
      var cv=document.getElementById('categoryViewSection'); if(cv) cv.classList.add('hidden');
      try{ setActiveCatNavBtn(null); }catch(e){}
      try{ shsSetHash(null); }catch(e){}
      shsRenderGroupView(origin.g, {scrollY: origin.scrollY});
      return;
    }
    return _shsCCVNav.apply(this, arguments);
  };
  closeCategoryView.__shsTreeHook = true;
  closeCategoryView.__shsNavHook = true;
}

/* الحلقات تعود إلى الشاشة التي فُتحت منها فعلاً (قسم / بحث / رئيسية)
   وإلى موضع التمرير فيها. كان المنطق القديم يستنتج الأصل من نصّ خانة
   البحث، فإن بقي فيها نصّ قديم عاد إلى نتائج بحث لم يكن فيها المستخدم. */
if(typeof backFromEpisodesToHome==='function' && !backFromEpisodesToHome.__shsNavHook){
  var _shsBackEpNav = backFromEpisodesToHome;
  backFromEpisodesToHome = function(fromHistory){
    try{
      if(!fromHistory && history.state && history.state.depth==='episodes'){ history.back(); return; }
    }catch(e){}
    var o = App.__epOrigin || null, y = App.__epScrollY;
    App.__epOrigin = null; App.__epScrollY = null;
    if(!o){ return _shsBackEpNav.apply(this, arguments); }        // لا أصل مسجّل — السلوك القديم
    document.getElementById('epSection').classList.add('hidden');
    App.currentSeriesId=null; App.currentSeriesName=''; App.currentSeriesPoster='';
    if(o==='search'){
      document.getElementById('searchViewSection').classList.remove('hidden');
      var q=(document.getElementById('searchInput')||{}).value||'';
      try{ shsSetHash(q.trim()? {q:q.trim().toLowerCase()} : null); }catch(e){}
    } else if(o==='category' && App.currentCategoryView){
      document.getElementById('categoryViewSection').classList.remove('hidden');
      try{ shsSetHash({c:App.currentCategoryView.id}); }catch(e){}
    } else {
      document.getElementById('netflixStyleSliders').classList.remove('hidden');
    ...
    if(typeof y==='number') window.scrollTo({top:y, behavior:'instant'}); else window.scrollTo({top:0,behavior:'smooth'});
  };
  backFromEpisodesToHome.__shsNavHook = true;
}
