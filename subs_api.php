...
case 'restream_diagnostics': {
    /* آخر السجل فقط، وهو متاح للمدير والجلسة المحمية بهذه الواجهة. */
    $file = LOG_DIR . '/restream-' . date('Y-m-d') . '.log';
    $rows = [];
    if (is_readable($file)) {
        ...
        foreach (array_slice($lines, -30) as $line) {
            ...
            $rows[] = [
                ...
                'ctx'  => is_array($entry['ctx'] ?? null) ? $entry['ctx'] : [],
            ];
        }
    }
    sOk(['entries' => $rows]);
...
case 'restream_set_limit': {
    require_once __DIR__ . '/functions/restream.php';
    $n = (int)($_POST['limit'] ?? 0);
    // القيد الحقيقي هو نطاق المدير، لا رقم اعتباطي — نفس سقف rsMaxChannels() تماماً
    // كي لا يُحفظ رقم ثم يُطبَّق رقم مختلف (هذا كان عطل "لا يمكنني تحديد الكمية").
    if ($n < 1 || $n > rsMaxChannelsCeiling()) sErr('bad_limit');
...
case 'restream_set_quality': {
    require_once __DIR__ . '/functions/restream.php';
    $q = (string)($_POST['quality'] ?? '');
    if (!rsSetRestreamQuality($q)) sErr('bad_quality');
    if (function_exists('logTo')) logTo('admin', "جودة إعادة ترميز الوسيط ← $q بواسطة $admin");
    sOk(['quality' => rsRestreamQuality()]);
    break;
}
