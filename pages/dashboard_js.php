...
function loadLoginLogs(){
    ...
    api({ajax_action:'get_login_logs'}).then(d => {
        if(d.success){
            ...
            /* الرسالة تُعرض في الجدول نفسه: alContainer ليس موجوداً في هذه
               الصفحة، فكان سبب الفشل يضيع ويبقى «خطأ في التحميل» وحده. */
            const msg = (d && d.error) ? String(d.error) : 'حدث خطأ غير معروف';
            $('llTbody').innerHTML = '<tr><td colspan="5" style="text-align:center;color:#ff6b6b;padding:18px;line-height:1.8">'
                + '<i class="fas fa-triangle-exclamation"></i> ' + esc(msg) + '</td></tr>';
        }
    }).catch(e => {
        $('llTbody').innerHTML = '<tr><td colspan="5" style="text-align:center;color:#ff6b6b;padding:18px">فشل الاتصال بالخادم — '
            + esc(String((e && e.message) || e || '')) + '</td></tr>';
    });
}
...
window.dashboardShortcuts = (function () {
    ...
    const destinations = {
        ...
        'storage-mgmt': { label: 'إدارة التخزين', icon: 'hard-drive' },
