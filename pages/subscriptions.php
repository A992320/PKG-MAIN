  ...
  <div style="margin-top:34px">
    ...
    <div style="background:var(--bg2,#181818);border:1px solid var(--br,#2a2a2a);border-radius:14px;padding:6px 20px">
      ...
      <div class="sb-row" style="align-items:center;gap:10px">
        ...
        <input type="number" id="rsLimitInput" min="1" step="1"
      ...
      <?php /* سقف جودة إعادة الترميز — تلقائي (نسخ حين يمكن) أو دقة محدّدة مضمونة */ ?>
      <div class="sb-row" style="align-items:center;gap:10px">
        <div style="width:48px;text-align:center;color:#B36BFF;font-size:1.15rem"><i class="fas fa-photo-video"></i></div>
        <div style="flex:1">
          <div class="sb-row-t"><?= $t["restream_quality_title"] ?? "سقف جودة البثّ" ?></div>
          <div class="sb-row-d" style="color:var(--t3,#8a8a94)">
            <?= $t["restream_quality_hint"] ?? "تلقائي = يُترك المصدر كما هو غالباً (الأسرع وبلا كلفة معالج). أي دقة محدّدة تُجبر الخادم على إعادة ترميز أي مصدر أعلى منها لضمان ألا يتجاوزها — يستهلك معالجاً أكثر لتلك القنوات مقابل نطاق أقل لكل مشاهد." ?>
          </div>
        </div>
        <select id="rsQualitySelect"
                style="padding:8px 10px;border-radius:8px;border:1px solid var(--br,#2a2a2a);background:var(--bg,#0f0f10);color:var(--t1,#fff);font-weight:700">
          <option value="auto"><?= $t["restream_quality_auto"] ?? "تلقائي" ?></option>
          <option value="480">480p</option>
          <option value="720">720p</option>
          <option value="1080">1080p</option>
          <option value="4k">4K</option>
        </select>
        <button class="sb-btn" onclick="rsSaveQuality()" id="rsQualitySaveBtn">
          <i class="fas fa-check"></i><?= $t["save"] ?? "حفظ" ?>
        </button>
      </div>
    ...
    <details style="margin-top:14px;max-width:760px;border:1px solid var(--br,#2a2a2a);border-radius:10px;padding:10px 13px">
      <summary style="cursor:pointer;font-weight:700;color:var(--t1,#fff)" onclick="setTimeout(rsLoadDiagnostics,0)">سجل تشخيص الوسيط</summary>
      <div id="rsDiagLog" style="margin-top:10px;color:var(--t3,#aaa);font-size:.78rem;line-height:1.7">افتح السجل لقراءة آخر محاولات التشغيل.</div>
    </details>
