...
try {
    /* تأكد ذاتي من وجود blocked_at: بعض التركيبات أنشأت الجدول قبل إضافة
       العمود، و CREATE TABLE IF NOT EXISTS لا يعدّل جدولاً موجوداً. */
    try { $pdo->exec("ALTER TABLE blocked_ips ADD COLUMN blocked_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP"); }
    catch (PDOException $eCol) { /* موجود مسبقاً */ }

    $__blockTtl = defined('IP_BLOCK_TTL_HOURS') ? (int) IP_BLOCK_TTL_HOURS : 24;
    try {
        $bStmt = $pdo->prepare("SELECT COUNT(*) FROM blocked_ips WHERE ip_address = ? AND blocked_at > (NOW() - INTERVAL {$__blockTtl} HOUR)");
        $bStmt->execute([$clientIp]);
    } catch (PDOException $eTtl) {
        /* شبكة أمان: لو تعذّر العمود لأي سبب (صلاحيات ALTER مثلاً) نرجع
           للفحص القديم بدل تعطيل صفحة الدخول بالكامل. */
        error_log('login blocked_ips TTL fallback: ' . $eTtl->getMessage());
